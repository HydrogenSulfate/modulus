...
def get_commit_files(repo_path: Path, branch_name: str, day_range: int=1,
    max_count: int=100):
    ...
    files = {}
    for commit in list(repo.iter_commits(rev=branch, since=
        f'{day_range}.days.ago', max_count=max_count)):
    ...
    for doc_file in userguide_path.rglob(file_pattern):
        for i, line in enumerate(open(doc_file)):
            for match in re.finditer(regex_pattern, line):
                ...
                elif doc_file_local in files[python_file]:
...
def create_gitlab_issue(commit_files, doc_files, gl_token: str):
    ...
    docs_repo_url = (
        'https://gitlab-master.nvidia.com/simnet/docs/-/blob/develop/')
    ...
    def file_desc(file_name, commit_time_stamp, commit_hash, doc_files):
        ...
        desc_str += (
            f":date: Editted: {commit_time.strftime('%Y-%m-%d %H:%M PST')}\n\n"
            )
    ...
    issue_title = (
        f'[Overwatch] Example files updated {todays_date.month}/{todays_date.day}/{todays_date.year}'
        )
    issue_desc = '### :robot: Overwatch Detected Files:\n\n'
    issue_desc += (
        'This is an automated issue created by CI Example Overwatch bot.\n\n')
    changed_files = False
    for commit_file_name in commit_files.keys():
        if commit_file_name in doc_files:
            issue_desc += file_desc(commit_file_name, commit_files[
                commit_file_name][0], commit_files[commit_file_name][1],
    ...
    gl = gitlab.Gitlab('https://gitlab-master.nvidia.com', private_token=
        gl_token)
    p = gl.projects.get('simnet/docs')
    p.issues.create({'title': issue_title, 'description': issue_desc,
        'labels': ['user guide', 'update']})


if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--gitlab-token', type=str, default=None, help=
        'Gitlab API token')
    p.add_argument('--day-range', type=int, default=1, help=
        'Day range to check commits')
    ...
    commit_files = get_commit_files(example_repo_path, 'develop', day_range
        =args['day_range'])
