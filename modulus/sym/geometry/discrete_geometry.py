...
class DiscreteGeometry(Geometry):
    ...
    def __init__(self, geometries, parameterization=Parameterization(),
        interior_epsilon=1e-06):

        def _sdf(list_sdf, discrete_parameterization, dims):

            def sdf(invar, params, compute_sdf_derivatives=False):
                outputs = {'sdf': np.full_like(next(iter(invar.values())),
                    np.nan)}
                if compute_sdf_derivatives:
                    for d in dims:
                        outputs['sdf' + diff_str + d] = np.full_like(next(
                            iter(invar.values())), -1000)
                for i, f in enumerate(list_sdf):
                    sdf_index = np.full_like(next(iter(invar.values())), True)
                    for key in discrete_parameterization.parameters:
                        expanded_d = np.tile(discrete_parameterization.
                            param_ranges[Parameter(key)][i:i + 1], (params[
                            key].shape[0], 1))
                        sdf_index = np.logical_and(sdf_index, params[key] ==
                            expanded_d)
                    sdf_indexed_invar = {key: value[sdf_index[:, 0], :] for
                        key, value in invar.items()}
                    sdf_indexed_params = {key: value[sdf_index[:, 0], :] for
                        key, value in params.items()}
                    computed_sdf = f(sdf_indexed_invar, sdf_indexed_params,
                        compute_sdf_derivatives)
                    for key, value in computed_sdf.items():
                        outputs[key][sdf_index[:, 0], :] = value
                return outputs
            return sdf
        new_sdf = _sdf([g.sdf for g in geometries], parameterization,
            geometries[0].dims)
        ...
        for g in geometries:
            new_curves += g.curves
        super().__init__(new_curves, new_sdf, dims=len(geometries[0].dims),
            bounds=bounds, parameterization=parameterization)
