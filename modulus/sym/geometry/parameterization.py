...
class Parameterization:
    ...
    def __init__(self, param_ranges: Dict[Parameter, Union[float, Tuple[
    ...
    def sample(self, nr_points: int, quasirandom: bool=False):
        ...
        quasirandom : bool
            ...
            Default is False.
        """
        return {str(key): value for key, value in _sample_ranges(nr_points,
            self.param_ranges, quasirandom).items()}
    ...
    def combine(cls, p1, p2):
        assert len(set(p1.parameters).intersection(set(p2.parameters))
            ) == 0, 'Combining parameterizations when they have overlapping parameters: p1 ' + str(
            p1) + ', p2 ' + str(p2)
...
class OrderedParameterization(Parameterization):
    ...
    def sample(self, nr_points: int, quasirandom: bool=False, sort:
        Optional='ascending'):
        ...
        """
        sample_dict = {}
        for key, value in _sample_ranges(nr_points, self.param_ranges,
            quasirandom).items():
            if key == self.key:
                if sort == 'ascending':
                    ...
                    raise ValueError(
                        "Sort must be one of None, 'ascending', or 'descending' (got {})"
                        .format(str(sort)))
...
class Bounds:
    ...
    def __init__(self, bound_ranges: Dict[Parameter, Tuple[Union[float,
        sympy.Basic], Union[float, sympy.Basic]]], parameterization:
        Parameterization=Parameterization()):
    ...
    def sample(self, nr_points: int, parameterization: Union[None,
        Parameterization]=None, quasirandom: bool=False):
        ...
        return {str(key): value for key, value in _sample_ranges(nr_points,
            computed_bound_ranges, quasirandom).items()}
    ...
    def volume(self, parameterization: Union[None, Parameterization]=None):
        ...
        """
        computed_bound_ranges = self._compute_bounds(parameterization)
        return np.prod([(value[1] - value[0]) for value in
            computed_bound_ranges.values()])

    def union(self, other):
        new_parameterization = self.parameterization.union(other.
            parameterization)
        new_bound_ranges = {}
        for (key, (lower_1, upper_1)), (lower_2, upper_2) in zip(self.
            bound_ranges.items(), other.bound_ranges.values()):
            if isinstance(lower_1, sympy.Basic) or isinstance(lower_2,
                sympy.Basic):
                new_lower = sympy.Min(lower_1, lower_2)
            elif isinstance(lower_1, (float, int)):
                new_lower = min(lower_1, lower_2)
            if isinstance(upper_1, sympy.Basic) or isinstance(upper_2,
                sympy.Basic):
    ...
    def scale(self, x, parameterization=Parameterization()):
        scaled_bound_ranges = {key: (lower * x, upper * x) for key, (lower,
            upper) in self.bound_ranges.items()}
        return Bounds(scaled_bound_ranges, self.parameterization.union(
            parameterization))

    def translate(self, xyz, parameterization=Parameterization()):
        translated_bound_ranges = {key: (lower + x, upper + x) for (key, (
            lower, upper)), x in zip(self.bound_ranges.items(), xyz)}
        return Bounds(translated_bound_ranges, self.parameterization.union(
            parameterization))

    def rotate(self, angle, axis, parameterization=Parameterization()):
        rotated_dims = [Parameter(key) for key in self.dims if key != axis]
        bounding_points = itertools.product(*[value for value in self.
            bound_ranges.values()])
        rotated_bounding_points = []
        for p in bounding_points:
            p = {Parameter(key): value for key, value in zip(self.dims, p)}
            rotated_p = {**p}
            rotated_p[rotated_dims[0]] = sympy.cos(angle) * p[rotated_dims[0]
                ] - sympy.sin(angle) * p[rotated_dims[1]]
            rotated_p[rotated_dims[1]] = sympy.sin(angle) * p[rotated_dims[0]
        ...
        return Bounds(rotated_bound_ranges, self.parameterization.union(
            parameterization))
    ...
    def _compute_bounds(self, parameterization=None, nr_sample=10000):
        ...
        for key, (lower, upper) in self.bound_ranges.items():
            if isinstance(lower, (float, int)):
                ...
                computed_lower = np.min(fn_lower(**parameterization.sample(
                    shape=nr_sample)))
            else:
                raise ValueError('Bound has non numeric or sympy values: ' +
            ...
            if isinstance(upper, (float, int)):
                ...
                computed_upper = np.max(fn_upper(**parameterization.sample(
                    shape=nr_sample)))
            else:
                raise ValueError('Bound has non numeric or sympy values: ' +
                    str(self.bound_ranges))
    ...
    def __str__(self):
        return 'bound_ranges: ' + str(self.bound_ranges
            ) + ' param_ranges: ' + str(self.parameterization)
    ...
    for key, value in ranges.items():
        if isinstance(value, tuple):
            if quasirandom:
                ...
                rand_param = (value[0] + (value[1] - value[0]) *
                    create_samples(indices, number_base=primes[prime_index]
                    ).reshape(-1, 1)).astype(float)
                prime_index += 1
            else:
                rand_param = np.random.uniform(value[0], value[1], size=(
                    batch_size, 1))
            ...
            raise ValueError('range type: ' + str(type(value)) +
