...
from .helper import _concat_numpy_dict_list, _sympy_sdf_to_sdf, _sympy_criteria_to_criteria, _sympy_func_to_func
...
class Geometry:
    ...
    def __init__(self, curves, sdf, dims, bounds, parameterization=
        Parameterization(), interior_epsilon=1e-06):
        self.curves = curves
        self.sdf = sdf
        self._dims = dims
        self.bounds = bounds
        self.parameterization = parameterization
        self.interior_epsilon = interior_epsilon
    ...
    def scale(self, x: Union[float, sympy.Basic], parameterization:
        ...
        def _scale_sdf(sdf, dims, x):
            ...
            def scale_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                computed_sdf = sdf(scaled_invar, params,
        ...
        return Geometry(new_curves, new_sdf, len(self.dims), new_bounds,
            new_parameterization, interior_epsilon=self.interior_epsilon)

    def translate(self, xyz: List[Union[float, sympy.Basic]],
        ...
        def _translate_sdf(sdf, dims, xyx):
            ...
            def translate_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                for i, key in enumerate(dims):
                    translated_invar[key] = translated_invar[key
                        ] - computed_translation[i]
                computed_sdf = sdf(translated_invar, params,
                    compute_sdf_derivatives)
                return computed_sdf
    ...
    def rotate(self, angle: Union[float, sympy.Basic], axis: str='z',
        center: Union[None, List[float]]=None, parameterization=
        Parameterization()):
        """
        Rotates geometry.
        ...
        def _rotate_sdf(sdf, dims, angle, axis, center):
            ...
            def rotate_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                _rotated_invar[rotated_dims[0]] = np.cos(computed_angle
                    ) * rotated_invar[rotated_dims[0]] + np.sin(computed_angle
                    ) * rotated_invar[rotated_dims[1]]
                _rotated_invar[rotated_dims[1]] = -np.sin(computed_angle
                    ) * rotated_invar[rotated_dims[0]] + np.cos(computed_angle
                    ) * rotated_invar[rotated_dims[1]]
                if center is not None:
                    for i, key in enumerate(dims):
                        _rotated_invar[key] = _rotated_invar[key] + center[i]
                computed_sdf = sdf(_rotated_invar, params,
                    compute_sdf_derivatives)
                return computed_sdf
            return rotate_sdf
    ...
    def repeat(self, spacing: float, repeat_lower: List[int], repeat_higher:
        List[int], center: Union[None, List[float]]=None):
        """
        ...
        def _repeat_sdf(sdf, dims, spacing, repeat_lower, repeat_higher, center
            ):
            ...
            def repeat_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                for d, rl, rh in zip(dims, repeat_lower, repeat_higher):
                    clamped_invar[d] = clamped_invar[d] - spacing * np.minimum(
                        np.maximum(np.around(clamped_invar[d] / spacing),
                        rl), rh)
                if center is not None:
                    for i, key in enumerate(dims):
                        clamped_invar[key] = clamped_invar[key] + center[i]
                computed_sdf = sdf(clamped_invar, params,
                    compute_sdf_derivatives)
                return computed_sdf
            return repeat_sdf
        new_sdf = _repeat_sdf(self.sdf, self.dims, spacing, repeat_lower,
            repeat_higher, center)
        new_bounds = self.bounds.copy()
        new_curves = []
        for t in itertools.product(*[list(range(rl, rh + 1)) for rl, rh in
            zip(repeat_lower, repeat_higher)]):
            new_bounds = new_bounds.union(self.bounds.translate([(spacing *
                a) for a in t]))
            new_curves += [c.translate([(spacing * a) for a in t]) for c in
                self.curves]
        return Geometry(new_curves, new_sdf, len(self.dims), new_bounds,
            self.parameterization.copy(), interior_epsilon=self.
            interior_epsilon)
    ...
    def boundary_criteria(self, invar, criteria=None, params={}):
        ...
        for key in self.dims:
            invar_normal_plus[key] = invar_normal_plus[key
                ] + self.interior_epsilon * invar_normal_plus['normal_' + key]
            invar_normal_minus[key] = invar_normal_minus[key
                ] - self.interior_epsilon * invar_normal_minus['normal_' + key]
        sdf_normal_plus = self.sdf(invar_normal_plus, params,
            compute_sdf_derivatives=False)['sdf']
        sdf_normal_minus = self.sdf(invar_normal_minus, params,
            compute_sdf_derivatives=False)['sdf']
    ...
    def sample_boundary(self, nr_points: int, criteria: Union[sympy.Basic,
        None]=None, parameterization: Union[Parameterization, None]=None,
        ...
        if criteria is not None:
            if isinstance(criteria, sympy.Basic):
                ...
                raise TypeError('criteria type is not supported: ' + str(
                    type(criteria)))
        ...
        def _boundary_criteria(criteria):
            ...
            def boundary_criteria(invar, params):
                return self.boundary_criteria(invar, criteria=criteria,
                    params=params)
            return boundary_criteria
        closed_boundary_criteria = _boundary_criteria(criteria)
        curve_areas = np.array([curve.approx_area(parameterization,
            criteria=closed_boundary_criteria) for curve in self.curves])
        ...
        points_per_curve = np.random.choice(curve_index, nr_points, p=
            curve_probabilities)
        points_per_curve, _ = np.histogram(points_per_curve, np.arange(len(
            self.curves) + 1) - 0.5)
        list_invar = []
        list_params = []
        for n, a, curve in zip(points_per_curve, curve_areas, self.curves):
            if n > 0:
                i, p = curve.sample(shape=n, criteria=
                    closed_boundary_criteria, parameterization=parameterization
                    )
    ...
    def sample_interior(self, nr_points: int, bounds: Union[Bounds, None]=
        None, criteria: Union[sympy.Basic, None]=None, parameterization:
        Union[Parameterization, None]=None, compute_sdf_derivatives: bool=
        False, quasirandom: bool=False):
        """
        ...
        nr_try = 0
        while True:
            local_invar = bounds.sample(nr_points, parameterization,
                quasirandom)
            local_params = parameterization.sample(nr_points, quasirandom)
            local_invar.update(self.sdf(local_invar, local_params,
                compute_sdf_derivatives=compute_sdf_derivatives))
            criteria_index = np.greater(local_invar['sdf'], 0)
            if criteria is not None:
                criteria_index = np.logical_and(criteria_index, criteria(
                    local_invar, local_params))
            ...
            for key in local_invar.keys():
                if key not in invar.keys():
                    invar[key] = local_invar[key]
                else:
                    invar[key] = np.concatenate([invar[key], local_invar[
                        key]], axis=0)
            for key in local_params.keys():
                if key not in params.keys():
                    params[key] = local_params[key]
                else:
                    params[key] = np.concatenate([params[key], local_params
                        [key]], axis=0)
        ...
        invar['area'] = np.full_like(next(iter(invar.values())), volume /
            nr_points)
        invar.update(params)
        return invar
    ...
    def __add__(self, other):
        ...
        def _add_sdf(sdf_1, sdf_2, dims):
            ...
            def add_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                computed_sdf = {}
                computed_sdf['sdf'] = np.maximum(computed_sdf_1['sdf'],
                    computed_sdf_2['sdf'])
        ...
        new_parameterization = self.parameterization.union(other.
            parameterization)
        new_bounds = self.bounds.union(other.bounds)
        return Geometry(self.curves + other.curves, new_sdf, len(self.dims),
            new_bounds, new_parameterization, interior_epsilon=self.
            interior_epsilon)
    ...
    def __sub__(self, other):
        ...
        def _sub_sdf(sdf_1, sdf_2, dims):
            ...
            def sub_sdf(invar, params, compute_sdf_derivatives=False):
                ...
                computed_sdf_2 = sdf_2(invar, params, compute_sdf_derivatives)
                computed_sdf = {}
                computed_sdf['sdf'] = np.minimum(computed_sdf_1['sdf'], -
                    computed_sdf_2['sdf'])
                if compute_sdf_derivatives:
                    for d in dims:
                        computed_sdf['sdf' + diff_str + d] = np.where(
