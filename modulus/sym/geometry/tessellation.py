    ...
    def __init__(self, mesh, airtight=True, parameterization=Parameterization()
        ...
        def _sample(mesh):
            ...
            def sample(nr_points, parameterization=Parameterization(),
                ...
                points_per_triangle = np.random.choice(triangle_index,
                    nr_points, p=triangle_probabilities)
                points_per_triangle, _ = np.histogram(points_per_triangle, 
                    np.arange(triangle_probabilities.shape[0] + 1) - 0.5)
                invar = {'x': [], 'y': [], 'z': [], 'normal_x': [],
                    'normal_y': [], 'normal_z': [], 'area': []}
                for index, nr_p in enumerate(points_per_triangle):
                    x, y, z = _sample_triangle(mesh.v0[index], mesh.v1[
                        index], mesh.v2[index], nr_p)
                    invar['x'].append(x)
                    invar['y'].append(y)
                    invar['z'].append(z)
                    normal_scale = np.linalg.norm(mesh.normals[index])
                    invar['normal_x'].append(np.full(x.shape, mesh.normals[
                        index, 0]) / normal_scale)
                    invar['normal_y'].append(np.full(x.shape, mesh.normals[
                        index, 1]) / normal_scale)
                    invar['normal_z'].append(np.full(x.shape, mesh.normals[
                        index, 2]) / normal_scale)
                    invar['area'].append(np.full(x.shape, triangle_areas[
                        index] / x.shape[0]))
                ...
                params = parameterization.sample(shape=nr_points,
                    quasirandom=quasirandom)
                return invar, params
            return sample
        curves = [Curve(_sample(mesh), dims=3, parameterization=
            parameterization)]
        ...
        def _sdf(triangles, airtight):
            ...
            def sdf(invar, params, compute_sdf_derivatives=False):
                ...
                if compute_sdf_derivatives:
                    ...
                    sdf_derivative = np.reshape(sdf_derivative, (
                        sdf_derivative.shape[0] // 3, 3))
                ...
                return outputs
            return sdf
        bounds = Bounds({Parameter('x'): (float(np.min(mesh.vectors[:, :, 0
            ])), float(np.max(mesh.vectors[:, :, 0]))), Parameter('y'): (
            float(np.min(mesh.vectors[:, :, 1])), float(np.max(mesh.vectors
            [:, :, 1]))), Parameter('z'): (float(np.min(mesh.vectors[:, :, 
            2])), float(np.max(mesh.vectors[:, :, 2])))}, parameterization=
            parameterization)
        super(Tessellation, self).__init__(curves, _sdf(mesh.vectors,
            airtight), dims=3, bounds=bounds, parameterization=parameterization
            )

    @classmethod
    def from_stl(cls, filename, airtight=True, parameterization=
        Parameterization()):
        """
...
def _sample_triangle(v0, v1, v2, nr_points):
