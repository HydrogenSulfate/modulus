""" base class for PDEs
"""
from sympy import Symbol, Function, init_printing, pprint, latex, preview, Matrix, Eq, Basic
...
class PDE(object):
    ...
    def pprint(self, print_latex=False):
        ...
        if print_latex:
            preview(Matrix([Eq(Function(name, real=True), eq) for name, eq in
                self.equations.items()]), mat_str='cases', mat_delim='')

    def subs(self, x, y):
        for name, eq in self.equations.items():
            self.equations[name] = eq.subs(x, y).doit()

    def make_nodes(self, create_instances: int=1, freeze_terms: Dict[str,
        List[int]]={}, detach_names: List[str]=[]):
        """
        ...
        if create_instances == 1:
            if bool(freeze_terms):
                ...
                freeze_terms = {}
            for name, eq in self.equations.items():
                nodes.append(Node.from_sympy(eq, str(name), freeze_terms,
                    detach_names))
        else:
            ...
            for i in range(create_instances):
                for name, eq in self.equations.items():
                    if str(name) + '_' + str(i) in freeze_terms.keys():
                        nodes.append(Node.from_sympy(eq, str(name) + '_' +
                            str(i), freeze_terms[str(name) + '_' + str(i)],
                            detach_names))
                    else:
                        print('No freeze terms found for instance: ' + str(
                            name) + '_' + str(i) + ', setting to empty')
                        nodes.append(Node.from_sympy(eq, str(name) + '_' +
                            str(i), [], detach_names))
        return nodes
