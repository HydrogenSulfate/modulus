...
class LinearElasticity(PDE):
    ...
    def __init__(self, E=None, nu=None, lambda_=None, mu=None, rho=1, dim=3,
        time=False):
        self.dim = dim
        self.time = time
        x, y, z = Symbol('x'), Symbol('y'), Symbol('z')
        normal_x, normal_y, normal_z = Symbol('normal_x'), Symbol('normal_y'
            ), Symbol('normal_z')
        t = Symbol('t')
        ...
        self.equations['stress_disp_xx'] = lambda_ * (u.diff(x) + v.diff(y) +
            w.diff(z)) + 2 * mu * u.diff(x) - sigma_xx
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_yy'] = lambda_ * (u.diff(x) + v.diff(y) +
            w.diff(z)) + 2 * mu * v.diff(y) - sigma_yy
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_zz'] = lambda_ * (u.diff(x) + v.diff(y) +
            w.diff(z)) + 2 * mu * w.diff(z) - sigma_zz
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_xy'] = mu * (u.diff(y) + v.diff(x)
            ) - sigma_xy
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_xz'] = mu * (u.diff(z) + w.diff(x)
            ) - sigma_xz
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_yz'] = mu * (v.diff(z) + w.diff(y)
            ) - sigma_yz
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        ...
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['navier_x'] = rho * u.diff(t).diff(t) - (lambda_ + mu
            ) * (u.diff(x) + v.diff(y) + w.diff(z)).diff(x) - mu * (u.diff(
            x).diff(x) + u.diff(y).diff(y) + u.diff(z).diff(z))
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['navier_y'] = rho * v.diff(t).diff(t) - (lambda_ + mu
            ) * (u.diff(x) + v.diff(y) + w.diff(z)).diff(y) - mu * (v.diff(
            x).diff(x) + v.diff(y).diff(y) + v.diff(z).diff(z))
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['navier_z'] = rho * w.diff(t).diff(t) - (lambda_ + mu
            ) * (u.diff(x) + v.diff(y) + w.diff(z)).diff(z) - mu * (w.diff(
            x).diff(x) + w.diff(y).diff(y) + w.diff(z).diff(z))
        if self.dim == 2:
...
class LinearElasticityPlaneStress(PDE):
    ...
    def __init__(self, E=None, nu=None, lambda_=None, mu=None, rho=1, time=
        False):
        self.time = time
        ...
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_xx'] = lambda_ * (u.diff(x) + v.diff(y) +
            w_z) + 2 * mu * u.diff(x) - sigma_xx
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_yy'] = lambda_ * (u.diff(x) + v.diff(y) +
            w_z) + 2 * mu * v.diff(y) - sigma_yy
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['stress_disp_xy'] = mu * (u.diff(y) + v.diff(x)
            ) - sigma_xy
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['equilibrium_x'] = rho * u.diff(t).diff(t) - (sigma_xx
            .diff(x) + sigma_xy.diff(y))
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        self.equations['equilibrium_y'] = rho * v.diff(t).diff(t) - (sigma_xy
            .diff(x) + sigma_yy.diff(y))
        self.equations['traction_x'
            ] = normal_x * sigma_xx + normal_y * sigma_xy
        self.equations['traction_y'
            ] = normal_x * sigma_xy + normal_y * sigma_yy
