...
class ZeroEquation(PDE):
    ...
    def __init__(self, nu, max_distance, rho=1, dim=3, time=True):
        self.dim = dim
        self.time = time
        ...
        mixing_length = Min(self.karman_constant * normal_distance, self.
            max_distance_ratio * self.max_distance)
        ...
        G = 2 * u.diff(x) ** 2 + 2 * v.diff(y) ** 2 + 2 * w.diff(z) ** 2 + (u
            .diff(y) + v.diff(x)) ** 2 + (u.diff(z) + w.diff(x)) ** 2 + (v.
            diff(z) + w.diff(y)) ** 2
        self.equations = {}
