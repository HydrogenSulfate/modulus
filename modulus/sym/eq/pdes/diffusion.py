...
class Diffusion(PDE):
    ...
    def __init__(self, T='T', D='D', Q=0, dim=3, time=True, mixed_form=False):
        ...
        if not self.mixed_form:
            ...
            self.equations['diffusion_' + self.T] = T.diff(t) - (D * T.diff(x)
                ).diff(x) - (D * T.diff(y)).diff(y) - (D * T.diff(z)).diff(z
                ) - Q
            ...
            if self.dim == 3:
                ...
                T_z = Number(0)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['diffusion_' + self.T] = T.diff(t) - (D * T_x).diff(
                x) - (D * T_y).diff(y) - (D * T_z).diff(z) - Q
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
...
class DiffusionInterface(PDE):
    ...
    def __init__(self, T_1, T_2, D_1, D_2, dim=3, time=True):
        ...
        self.equations = {}
        self.equations['diffusion_interface_dirichlet_' + self.T_1 + '_' +
            self.T_2] = T_1 - T_2
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        flux_1 = self.D_1 * (normal_x * T_1.diff(x) + normal_y * T_1.diff(y
            ) + normal_z * T_1.diff(z))
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        flux_2 = self.D_2 * (normal_x * T_2.diff(x) + normal_y * T_2.diff(y
            ) + normal_z * T_2.diff(z))
        self.equations['diffusion_interface_neumann_' + self.T_1 + '_' +
            self.T_2] = flux_1 - flux_2
