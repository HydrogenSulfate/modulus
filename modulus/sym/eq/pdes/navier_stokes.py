...
class NavierStokes(PDE):
    ...
    def __init__(self, nu, rho=1, dim=3, time=True, mixed_form=False):
        ...
        self.equations['continuity'] = rho.diff(t) + (rho * u).diff(x) + (rho *
            v).diff(y) + (rho * w).diff(z)
        if not self.mixed_form:
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            curl = Number(0) if rho.diff(x) == 0 else u.diff(x) + v.diff(y
                ) + w.diff(z)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['momentum_x'] = (rho * u).diff(t) + (u * (rho *
                u).diff(x) + v * (rho * u).diff(y) + w * (rho * u).diff(z) +
                rho * u * curl) + p.diff(x) - (-2 / 3 * mu * curl).diff(x) - (
                mu * u.diff(x)).diff(x) - (mu * u.diff(y)).diff(y) - (mu *
                u.diff(z)).diff(z) - mu * curl.diff(x) - mu.diff(x) * u.diff(x
                ) - mu.diff(y) * v.diff(x) - mu.diff(z) * w.diff(x)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['momentum_y'] = (rho * v).diff(t) + (u * (rho *
                v).diff(x) + v * (rho * v).diff(y) + w * (rho * v).diff(z) +
                rho * v * curl) + p.diff(y) - (-2 / 3 * mu * curl).diff(y) - (
                mu * v.diff(x)).diff(x) - (mu * v.diff(y)).diff(y) - (mu *
                v.diff(z)).diff(z) - mu * curl.diff(y) - mu.diff(x) * u.diff(y
                ) - mu.diff(y) * v.diff(y) - mu.diff(z) * w.diff(y)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['momentum_z'] = (rho * w).diff(t) + (u * (rho *
                w).diff(x) + v * (rho * w).diff(y) + w * (rho * w).diff(z) +
                rho * w * curl) + p.diff(z) - (-2 / 3 * mu * curl).diff(z) - (
                mu * w.diff(x)).diff(x) - (mu * w.diff(y)).diff(y) - (mu *
                w.diff(z)).diff(z) - mu * curl.diff(z) - mu.diff(x) * u.diff(z
                ) - mu.diff(y) * v.diff(z) - mu.diff(z) * w.diff(z)
            if self.dim == 2:
            ...
            self.equations['momentum_x'] = (rho * u).diff(t) + (u * (rho *
                u.diff(x)) + v * (rho * u.diff(y)) + w * (rho * u.diff(z)) +
                rho * u * curl) + p.diff(x) - (-2 / 3 * mu * curl).diff(x) - (
                mu * u_x).diff(x) - (mu * u_y).diff(y) - (mu * u_z).diff(z
                ) - mu * curl.diff(x) - mu.diff(x) * u.diff(x) - mu.diff(y
                ) * v.diff(x) - mu.diff(z) * w.diff(x)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['momentum_y'] = (rho * v).diff(t) + (u * (rho *
                v.diff(x)) + v * (rho * v.diff(y)) + w * (rho * v.diff(z)) +
                rho * v * curl) + p.diff(y) - (-2 / 3 * mu * curl).diff(y) - (
                mu * v_x).diff(x) - (mu * v_y).diff(y) - (mu * v_z).diff(z
                ) - mu * curl.diff(y) - mu.diff(x) * u.diff(y) - mu.diff(y
                ) * v.diff(y) - mu.diff(z) * w.diff(y)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['momentum_z'] = (rho * w).diff(t) + (u * (rho *
                w.diff(x)) + v * (rho * w.diff(y)) + w * (rho * w.diff(z)) +
                rho * w * curl) + p.diff(z) - (-2 / 3 * mu * curl).diff(z) - (
                mu * w_x).diff(x) - (mu * w_y).diff(y) - (mu * w_z).diff(z
                ) - mu * curl.diff(z) - mu.diff(x) * u.diff(z) - mu.diff(y
                ) * v.diff(z) - mu.diff(z) * w.diff(z)
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
...
class FluxContinuity(PDE):
    ...
    def __init__(self, T='T', D='D', rho=1, vec=['u', 'v', 'w']):
        ...
        for v, n, g in zip(vec, normal, gradient):
            self.equations[str(T) + '_flux'] += Symbol(v
                ) * n * rho * T - rho * D * n * g
