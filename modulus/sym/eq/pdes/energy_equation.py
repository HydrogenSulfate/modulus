...
class EnergyFluid(PDE):
    ...
    def __init__(self, cp='cp', kappa='kappa', rho='rho', nu='nu',
        visc_heating=False, dim=3, time=False, mixed_form=False):
        self.dim = dim
        self.time = time
        self.nu = nu
        self.rho = rho
        ...
        if visc_heating == True:
            for i, j in zip(range(0, 3), range(0, 3)):
                ...
                visc_h = visc_h + (vel[i].diff(coord[j]) * vel[i].diff(
                    coord[j]) + vel[i].diff(coord[j]) * vel[j].diff(coord[i
                    ]) - 2 / 3 * vel[i].diff(coord[i]) * vel[j].diff(coord[j]))
        visc_h = nu * rho * visc_h
        """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        p_work = 0 * x if type(self.rho) == float else p.diff(t) + u * p.diff(x
            ) + v * p.diff(y) + w * p.diff(z)
        self.equations = {}
        if not self.mixed_form:
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
            self.equations['temperauture_fluid'] = rho * cp * (T.diff(t) + 
                u * T.diff(x) + v * T.diff(y) + w * T.diff(z)
                ) - kappa * T.diff(x).diff(x) - kappa * T.diff(y).diff(y
                ) - kappa * T.diff(z).diff(z) - p_work - visc_h
            ...
            self.equations['temperauture_fluid'] = rho * cp * (T.diff(t) + 
                u * T.diff(x) + v * T.diff(y) + w * T.diff(z)
                ) - kappa * T_x.diff(x) - kappa * T_y.diff(y
                ) - kappa * T_z.diff(z) - p_work - visc_h
            """Class Method: *.diff, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
