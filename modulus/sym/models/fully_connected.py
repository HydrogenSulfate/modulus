...
class FullyConnectedArchCore(paddle.nn.Layer):

    def __init__(self, in_features: int=512, layer_size: int=512,
        out_features: int=512, nr_layers: int=6, skip_connections: bool=
        False, activation_fn: Activation=Activation.SILU,
        adaptive_activations: bool=False, weight_norm: bool=True,
        conv_layers: bool=False) ->None:
        super().__init__()
        ...
        if adaptive_activations:
            out_56 = paddle.create_parameter(shape=paddle.ones(shape=[1]).
                shape, dtype=paddle.ones(shape=[1]).numpy().dtype,
                default_initializer=paddle.nn.initializer.Assign(paddle.
                ones(shape=[1])))
            out_56.stop_gradient = not True
            activation_par = out_56
        else:
            activation_par = None
        if not isinstance(activation_fn, list):
            activation_fn = [activation_fn] * nr_layers
        if len(activation_fn) < nr_layers:
            activation_fn = activation_fn + [activation_fn[-1]] * (nr_layers -
                len(activation_fn))
        self.layers = paddle.nn.LayerList()
        layer_in_features = in_features
        for i in range(nr_layers):
            self.layers.append(fc_layer(layer_in_features, layer_size,
                activation_fn[i], weight_norm, activation_par))
            layer_in_features = layer_size
        self.final_layer = fc_layer(in_features=layer_size, out_features=
            out_features, activation_fn=Activation.IDENTITY, weight_norm=
            False, activation_par=None)
    ...
    def get_weight_list(self):
        weights = [layer.conv.weight for layer in self.layers] + [self.
            final_layer.conv.weight]
        biases = [layer.conv.bias for layer in self.layers] + [self.
            final_layer.conv.bias]
...
class FullyConnectedArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        activation_fn=Activation.SILU, periodicity: Union[Dict[str, Tuple[
        float, float]], None]=None, skip_connections: bool=False,
        adaptive_activations: bool=False, weight_norm: bool=True) ->None:
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            detach_keys=detach_keys, periodicity=periodicity)
        if self.periodicity is not None:
            in_features = sum([x.size for x in self.input_keys if x.name not in
                list(periodicity.keys())]) + +sum([(2 * x.size) for x in
                self.input_keys if x.name in list(periodicity.keys())])
        else:
            in_features = sum(self.input_key_dict.values())
        out_features = sum(self.output_key_dict.values())
        self._impl = FullyConnectedArchCore(in_features, layer_size,
            out_features, nr_layers, skip_connections, activation_fn,
            adaptive_activations, weight_norm)
...
class ConvFullyConnectedArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        detach_keys: List[Key]=[], layer_size: int=512, nr_layers: int=6,
        activation_fn=Activation.SILU, skip_connections: bool=False,
        adaptive_activations: bool=False) ->None:
        super().__init__(input_keys=input_keys, output_keys=output_keys,
        ...
        self._impl = FullyConnectedArchCore(in_features, layer_size,
            out_features, nr_layers, skip_connections, activation_fn,
            adaptive_activations, weight_norm=False, conv_layers=True)
