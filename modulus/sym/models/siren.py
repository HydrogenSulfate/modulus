...
class SirenArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        first_omega: float=30.0, omega: float=30.0, normalization: Dict[str,
        ...
        layers_list = []
        layers_list.append(layers.SirenLayer(in_features, layer_size,
            layers.SirenLayerType.FIRST, first_omega))
        for _ in range(nr_layers - 1):
            layers_list.append(layers.SirenLayer(layer_size, layer_size,
                layers.SirenLayerType.HIDDEN, omega))
        layers_list.append(layers.SirenLayer(layer_size, out_features,
            layers.SirenLayerType.LAST, omega))
        self.layers = paddle.nn.Sequential(*layers_list)
