...
from modulus.sym.models.afno.distributed.mappings import scatter_to_matmul_parallel_region, gather_from_matmul_parallel_region
from modulus.sym.models.afno.distributed.layers import trunc_normal_, DropPath
from modulus.sym.models.afno.distributed.layers import DistributedPatchEmbed, DistributedMLP, DistributedAFNO2D
import logging
logger = logging.getLogger(__name__)


class DistributedBlock(paddle.nn.Layer):

    def __init__(self, h, w, dim, mlp_ratio=4.0, drop=0.0, drop_path=0.0,
        act_layer=paddle.nn.GELU, norm_layer=paddle.nn.LayerNorm,
        double_skip=True, num_blocks=8, sparsity_threshold=0.01,
        hard_thresholding_fraction=1.0, input_is_matmul_parallel=False,
        ...
        self.filter = DistributedAFNO2D(dim, num_blocks, sparsity_threshold,
            hard_thresholding_fraction, input_is_matmul_parallel=True,
            output_is_matmul_parallel=True)
        self.drop_path = DropPath(drop_path
            ) if drop_path > 0.0 else paddle.nn.Identity()
        self.norm2 = norm_layer((h, w))
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = DistributedMLP(in_features=dim, hidden_features=
            mlp_hidden_dim, act_layer=act_layer, drop=drop,
            input_is_matmul_parallel=True, output_is_matmul_parallel=True)
...
class DistributedAFNONet(paddle.nn.Layer):

    def __init__(self, img_size=(720, 1440), patch_size=(16, 16), in_chans=
        2, out_chans=2, embed_dim=768, depth=12, mlp_ratio=4.0, drop_rate=
        0.0, drop_path_rate=0.0, num_blocks=16, sparsity_threshold=0.01,
        hard_thresholding_fraction=1.0, input_is_matmul_parallel=False,
        output_is_matmul_parallel=False):
        super().__init__()
        ...
        self.patch_embed = DistributedPatchEmbed(img_size=img_size,
            patch_size=self.patch_size, in_chans=self.in_chans, embed_dim=
            embed_dim, input_is_matmul_parallel=self.
            input_is_matmul_parallel, output_is_matmul_parallel=True)
        num_patches = self.patch_embed.num_patches
        self.embed_dim_local = self.embed_dim // matmul_comm_size
        out_91 = paddle.create_parameter(shape=paddle.zeros(shape=[1, self.
            embed_dim_local, num_patches]).shape, dtype=paddle.zeros(shape=
            [1, self.embed_dim_local, num_patches]).numpy().dtype,
            default_initializer=paddle.nn.initializer.Assign(paddle.zeros(
            shape=[1, self.embed_dim_local, num_patches])))
        out_91.stop_gradient = not True
        self.pos_embed = out_91
        ...
        for i in range(0, depth):
            ...
            blks.append(DistributedBlock(h=self.h, w=self.w, dim=embed_dim,
                mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i],
                norm_layer=norm_layer, num_blocks=self.num_blocks,
                sparsity_threshold=sparsity_threshold,
                hard_thresholding_fraction=hard_thresholding_fraction,
                input_is_matmul_parallel=input_is_matmul_parallel,
                output_is_matmul_parallel=output_is_matmul_parallel))
        self.blocks = paddle.nn.LayerList(sublayers=blks)
        if self.output_is_matmul_parallel:
            self.out_chans_local = (self.out_chans + matmul_comm_size - 1
                ) // matmul_comm_size
        else:
            self.out_chans_local = self.out_chans
        self.head = paddle.nn.Conv2D(in_channels=self.embed_dim,
            out_channels=self.out_chans_local * self.patch_size[0] * self.
            patch_size[1], kernel_size=1, bias_attr=False)
    ...
    def forward(self, x):
        ...
        if self.output_is_matmul_parallel:
            ...
            for param in self.head.parameters():
                paddle.distributed.broadcast(tensor=param, src=0, group=
                    DistributedManager().group('model_parallel'))
        ...
        xv = x.view(b, self.patch_size[0], self.patch_size[1], -1, self.h,
            self.w)
        xvt = paddle.transpose(x=xv, perm=(0, 3, 4, 1, 5, 2))
        """Class Method: *.view, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
        x = xvt.view(b, -1, self.h * self.patch_size[0], self.w * self.
            patch_size[1])
        return x
...
class DistributedAFNOArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        int=16, embed_dim: int=256, depth: int=4, num_blocks: int=4,
        channel_parallel_inputs: bool=False, channel_parallel_outputs: bool
        =False) ->None:
        ...
        if channel_parallel_inputs:
            assert in_channels % comm_size == 0, 'Error, in_channels needs to be divisible by model_parallel size'
        self._impl = DistributedAFNONet(img_size=img_shape, patch_size=(
            patch_size, patch_size), in_chans=in_channels, out_chans=
            out_channels, embed_dim=embed_dim, depth=depth, num_blocks=
            num_blocks, input_is_matmul_parallel=False,
            output_is_matmul_parallel=False)
