...
class MultiscaleFourierNetArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        detach_keys: List[Key]=[], frequencies=(('axis', [i for i in range(
        10)]),), frequencies_params=(('axis', [i for i in range(10)]),),
        activation_fn=layers.Activation.SILU, layer_size: int=512,
        nr_layers: int=6, skip_connections: bool=False, weight_norm: bool=
        True, adaptive_activations: bool=False) ->None:
        ...
        if adaptive_activations:
            out_77 = paddle.create_parameter(shape=paddle.ones(shape=[1]).
                shape, dtype=paddle.ones(shape=[1]).numpy().dtype,
                default_initializer=paddle.nn.initializer.Assign(paddle.
                ones(shape=[1])))
            out_77.stop_gradient = not True
            activation_par = out_77
        else:
            activation_par = None
        in_features = in_features_xyzt + in_features_params
        ...
        if in_features_xyzt > 0:
            self.fourier_layers_xyzt = paddle.nn.LayerList()
            for idx in range(self.num_freqs):
                self.fourier_layers_xyzt.append(layers.FourierLayer(
                    in_features=in_features_xyzt, frequencies=frequencies[idx])
                    )
            in_features += self.fourier_layers_xyzt[0].out_features()
        else:
            self.fourier_layers_xyzt = None
        if in_features_params > 0:
            self.fourier_layers_params = paddle.nn.LayerList()
            for idx in range(self.num_freqs):
                self.fourier_layers_params.append(layers.FourierLayer(
                    in_features=in_features_params, frequencies=
                    frequencies_params[idx]))
        ...
        for i in range(nr_layers):
            self.fc_layers.append(layers.FCLayer(layer_in_features,
                layer_size, activation_fn, weight_norm, activation_par))
            layer_in_features = layer_size
        self.final_layer = layers.FCLayer(in_features=layer_size * self.
            num_freqs, out_features=out_features, activation_fn=layers.
            Activation.IDENTITY, weight_norm=False, activation_par=None)
    ...
    def _tensor_forward(self, x: paddle.Tensor) ->paddle.Tensor:
        ...
        old_x = x
        fc_outputs = []
        _len = len(self.fourier_layers_xyzt
            ) if self.fourier_layers_xyzt is not None else len(self.
            fourier_layers_params)
        zip_fourier_layers_xyzt = (self.fourier_layers_xyzt if self.
            fourier_layers_xyzt is not None else [None] * _len)
        zip_fourier_layers_params = (self.fourier_layers_params if self.
            fourier_layers_params is not None else [None] * _len)
            ...
            for i, layer in enumerate(self.fc_layers):
                ...
                if self.skip_connections and i % 2 == 0:
                    if x_skip is not None:
                        ...
                        x_skip = x
            fc_outputs.append(x)
        x = paddle.concat(x=fc_outputs, axis=-1)
