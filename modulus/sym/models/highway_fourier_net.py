...
class HighwayFourierNetArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        True, adaptive_activations: bool=False, transform_fourier_features:
        bool=True, project_fourier_features: bool=False) ->None:
        ...
        if adaptive_activations:
            out_76 = paddle.create_parameter(shape=paddle.ones(shape=[1]).
                shape, dtype=paddle.ones(shape=[1]).numpy().dtype,
                default_initializer=paddle.nn.initializer.Assign(paddle.
                ones(shape=[1])))
            out_76.stop_gradient = not True
            activation_par = out_76
        else:
            activation_par = None
        in_features = in_features_xyzt + in_features_params
        ...
        self.fc_t = layers.FCLayer(transformer_in_features, layer_size,
            activation_fn=layers.Activation.SIGMOID, weight_norm=
            weight_norm, activation_par=activation_par)
        self.fc_v = layers.FCLayer(projector_in_features, layer_size,
            activation_fn=layers.Activation.IDENTITY, weight_norm=
            weight_norm, activation_par=activation_par)
        ...
        self.final_layer = layers.FCLayer(layer_size, out_features,
            activation_fn=layers.Activation.IDENTITY, weight_norm=False,
            activation_par=None)
