...
class ModifiedFourierNetArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        if adaptive_activations:
            out_57 = paddle.create_parameter(shape=paddle.ones(shape=[1]).
                shape, dtype=paddle.ones(shape=[1]).numpy().dtype,
                default_initializer=paddle.nn.initializer.Assign(paddle.
                ones(shape=[1])))
            out_57.stop_gradient = not True
            activation_par = out_57
        else:
            activation_par = None
        ...
        self.fc_u = layers.FCLayer(in_features=in_features, out_features=
            layer_size, activation_fn=activation_fn, weight_norm=
            weight_norm, activation_par=activation_par)
        self.fc_v = layers.FCLayer(in_features=in_features, out_features=
            layer_size, activation_fn=activation_fn, weight_norm=
            weight_norm, activation_par=activation_par)
        self.fc_0 = layers.FCLayer(in_features, layer_size, activation_fn,
            weight_norm, activation_par=activation_par)
        self.fc_layers = paddle.nn.LayerList()
        for i in range(nr_layers - 1):
            self.fc_layers.append(layers.FCLayer(layer_size, layer_size,
                activation_fn, weight_norm, activation_par=activation_par))
