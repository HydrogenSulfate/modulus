...
class TinyCudaNNArchCore(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        periodicity: Union[Dict[str, Tuple[float, float]], None]=None,
        detach_keys: List[Key]=[], layer_size: int=64, nr_layers: int=2,
        activation_fn=layers.Activation.SIGMOID, fully_fused: bool=True,
        encoding_config: Optional[Dict]=None) ->None:
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            detach_keys=detach_keys, periodicity=periodicity)
        supported_activations = {layers.Activation.RELU: 'ReLU', layers.
            Activation.SIGMOID: 'Sigmoid', layers.Activation.SQUAREPLUS:
            'Squareplus', layers.Activation.SOFTPLUS: 'Softplus', layers.
        ...
        network_config = {'otype': 'FullyFusedMLP' if fully_fused else
            'CutlassMLP', 'activation': supported_activations[activation_fn
            ], 'output_activation': 'None', 'n_neurons': layer_size,
            'n_hidden_layers': nr_layers}
        if encoding_config is not None:
            self._impl = tcnn.NetworkWithInputEncoding(in_features,
                out_features, encoding_config, network_config)
        else:
...
class FusedMLPArch(TinyCudaNNArchCore):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            periodicity=periodicity, detach_keys=detach_keys, layer_size=
            layer_size, nr_layers=nr_layers, activation_fn=activation_fn,
            fully_fused=fully_fused, encoding_config=None)
...
class FusedFourierNetArch(TinyCudaNNArchCore):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        detach_keys: List[Key]=[], layer_size: int=64, nr_layers: int=2,
        activation_fn=layers.Activation.SIGMOID, fully_fused: bool=True,
        n_frequencies: int=12) ->None:
        encoding_config = {'otype': 'Frequency', 'n_frequencies': n_frequencies
            }
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            periodicity=periodicity, detach_keys=detach_keys, layer_size=
            layer_size, nr_layers=nr_layers, activation_fn=activation_fn,
            fully_fused=fully_fused, encoding_config=encoding_config)
...
class FusedGridEncodingNetArch(TinyCudaNNArchCore):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        ...
        detach_keys: List[Key]=[], layer_size: int=64, nr_layers: int=2,
        activation_fn=layers.Activation.SIGMOID, fully_fused: bool=True,
        indexing: str='Hash', n_levels: int=16, n_features_per_level: int=2,
        log2_hashmap_size: int=19, base_resolution: int=16, per_level_scale:
        float=2.0, interpolation: str='Smoothstep') ->None:
        ...
        if interpolation not in ['Nearest', 'Linear', 'Smoothstep']:
            raise ValueError(
                f'interpolation type {interpolation} not supported')
        encoding_config = {'otype': 'Grid', 'type': indexing, 'n_levels':
            n_levels, 'n_features_per_level': 2, 'log2_hashmap_size': 19,
            'base_resolution': 16, 'per_level_scale': 2.0, 'interpolation':
            interpolation}
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            periodicity=periodicity, detach_keys=detach_keys, layer_size=
            layer_size, nr_layers=nr_layers, activation_fn=activation_fn,
