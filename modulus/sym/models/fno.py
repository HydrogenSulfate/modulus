...
from modulus.sym.models.layers.spectral_layers import calc_latent_derivatives, first_order_pino_grads, second_order_pino_grads
...
class FNO1DEncoder(paddle.nn.Layer):

    def __init__(self, in_channels: int=1, nr_fno_layers: int=4,
        fno_layer_size: int=32, fno_modes: Union[int, List[int]]=16,
        padding: Union[int, List[int]]=8, padding_type: str='constant',
        activation_fn: Activation=Activation.GELU, coord_features: bool=True
        ) ->None:
        super().__init__()
        ...
        self.spconv_layers = paddle.nn.LayerList()
        ...
        for _ in range(self.nr_fno_layers):
            self.spconv_layers.append(layers.SpectralConv1d(self.fno_width,
                self.fno_width, fno_modes[0]))
            self.conv_layers.append(paddle.nn.Conv1D(in_channels=self.
                fno_width, out_channels=self.fno_width, kernel_size=1))
    ...
    def forward(self, x: paddle.Tensor) ->paddle.Tensor:
        if self.coord_features:
            ...
            x = paddle.concat(x=(x, coord_feat), axis=1)
        x = self.lift_layer(x)
        x = paddle.nn.functional.pad(x, (0, self.pad[0]), mode=self.padding_type
            )
    ...
    def meshgrid(self, shape: List[int], device: str):
        ...
        grid_x = paddle.linspace(start=0, stop=1, num=size_x).astype('float32')
        grid_x = grid_x.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1])
        return grid_x


class FNO2DEncoder(paddle.nn.Layer):
    ...
    def __init__(self, in_channels: int=1, nr_fno_layers: int=4,
        ...
        for _ in range(self.nr_fno_layers):
            self.spconv_layers.append(layers.SpectralConv2d(self.fno_width,
                self.fno_width, fno_modes[0], fno_modes[1]))
            self.conv_layers.append(paddle.nn.Conv2D(in_channels=self.
                fno_width, out_channels=self.fno_width, kernel_size=1))
        if isinstance(padding, int):
    ...
    def forward(self, x: paddle.Tensor) ->paddle.Tensor:
        assert x.dim(
            ) == 4, 'Only 4D tensors [batch, in_channels, grid_x, grid_y] accepted for 2D FNO'
        if self.coord_features:
            coord_feat = self.meshgrid(list(x.shape), x.place)
            x = paddle.concat(x=(x, coord_feat), axis=1)
        x = self.lift_layer(x)
        x = paddle.nn.functional.pad(x, (0, self.pad[0], 0, self.pad[1]),
    ...
    def meshgrid(self, shape: List[int], device: str):
        ...
        grid_x = paddle.linspace(start=0, stop=1, num=size_x).astype('float32')
        grid_y = paddle.linspace(start=0, stop=1, num=size_y).astype('float32')
        grid_x, grid_y = paddle.meshgrid(grid_x, grid_y)
        grid_x = grid_x.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1, 1])
        grid_y = grid_y.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1, 1])
        return paddle.concat(x=(grid_x, grid_y), axis=1)


class FNO3DEncoder(paddle.nn.Layer):
    ...
    def __init__(self, in_channels: int=1, nr_fno_layers: int=4,
        ...
        for _ in range(self.nr_fno_layers):
            self.spconv_layers.append(layers.SpectralConv3d(self.fno_width,
                self.fno_width, fno_modes[0], fno_modes[1], fno_modes[2]))
            self.conv_layers.append(paddle.nn.Conv3D(in_channels=self.
                fno_width, out_channels=self.fno_width, kernel_size=1))
        if isinstance(padding, int):
    ...
    def forward(self, x: paddle.Tensor) ->paddle.Tensor:
        if self.coord_features:
            ...
            x = paddle.concat(x=(x, coord_feat), axis=1)
        x = self.lift_layer(x)
        x = paddle.nn.functional.pad(x, (0, self.pad[0], 0, self.pad[1], 0,
            self.pad[2]), mode=self.padding_type)
        for k, conv_w in enumerate(zip(self.conv_layers, self.spconv_layers)):
            conv, w = conv_w
            if k < len(self.conv_layers) - 1:
                x = self.activation_fn(conv(x) + w(x))
    ...
    def meshgrid(self, shape: List[int], device: str):
        ...
        grid_x = paddle.linspace(start=0, stop=1, num=size_x).astype('float32')
        grid_y = paddle.linspace(start=0, stop=1, num=size_y).astype('float32')
        grid_z = paddle.linspace(start=0, stop=1, num=size_z).astype('float32')
        grid_x, grid_y, grid_z = paddle.meshgrid(grid_x, grid_y, grid_z)
        grid_x = grid_x.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1, 1, 1])
        grid_y = grid_y.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1, 1, 1])
        grid_z = grid_z.unsqueeze(axis=0).unsqueeze(axis=0).tile(repeat_times
            =[bsize, 1, 1, 1, 1])
        return paddle.concat(x=(grid_x, grid_y, grid_z), axis=1)


def grid_to_points1d(vars_dict: Dict[str, Tensor]):
    for var, value in vars_dict.items():
        value = paddle.transpose(x=value, perm=(0, 2, 1))
...
def points_to_grid1d(vars_dict: Dict[str, Tensor], shape: List[int]):
    for var, value in vars_dict.items():
        ...
        vars_dict[var] = paddle.transpose(x=value, perm=(0, 2, 1))
    return vars_dict


def grid_to_points2d(vars_dict: Dict[str, Tensor]):
    for var, value in vars_dict.items():
        value = paddle.transpose(x=value, perm=(0, 2, 3, 1))
        vars_dict[var] = value.reshape(-1, value.shape[-1])
    return vars_dict
...
def points_to_grid2d(vars_dict: Dict[str, Tensor], shape: List[int]):
    for var, value in vars_dict.items():
        ...
        vars_dict[var] = paddle.transpose(x=value, perm=(0, 3, 1, 2))
    return vars_dict


def grid_to_points3d(vars_dict: Dict[str, Tensor]):
    for var, value in vars_dict.items():
        value = paddle.transpose(x=value, perm=(0, 2, 3, 4, 1))
        vars_dict[var] = value.reshape(-1, value.shape[-1])
    return vars_dict
...
def points_to_grid3d(vars_dict: Dict[str, Tensor], shape: List[int]):
    for var, value in vars_dict.items():
        value = value.reshape(shape[0], shape[2], shape[3], shape[4], value
            .shape[-1])
        vars_dict[var] = paddle.transpose(x=value, perm=(0, 4, 1, 2, 3))
    return vars_dict
...
class FNOArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], dimension: int, decoder_net:
        Arch, detach_keys: List[Key]=[], nr_fno_layers: int=4, fno_modes:
        Union[int, List[int]]=16, padding: int=8, padding_type: str=
        'constant', activation_fn: Activation=Activation.GELU,
        coord_features: bool=True) ->None:
        super().__init__(input_keys=input_keys, output_keys=[], detach_keys
            =detach_keys)
        ...
        assert len(self.latent_key
            ) == 1, 'FNO decoder network should only have a single input key'
        ...
        if self.dimension == 1:
            ...
            self.grid_to_points = grid_to_points1d
            self.points_to_grid = points_to_grid1d
        elif self.dimension == 2:
            FNOModel = FNO2DEncoder
            self.grid_to_points = grid_to_points2d
            self.points_to_grid = points_to_grid2d
        elif self.dimension == 3:
            FNOModel = FNO3DEncoder
            self.grid_to_points = grid_to_points3d
            self.points_to_grid = points_to_grid3d
        else:
            raise NotImplementedError(
                'Invalid dimensionality. Only 1D, 2D and 3D FNO implemented')
        self.spec_encoder = FNOModel(in_channels, nr_fno_layers=self.
            nr_fno_layers, fno_layer_size=self.fno_layer_size, fno_modes=
            self.fno_modes, padding=self.padding, padding_type=self.
            padding_type, activation_fn=self.activation_fn, coord_features=
            self.coord_features)

    def add_pino_gradients(self, derivatives: List[Key], domain_length:
        List[float]=[1.0, 1.0]) ->None:
        ...
        assert len(domain_length
            ) == self.dimension, 'Domain length must be same length as the dimension of the model'
        self.domain_length = domain_length
        logger.warning(
            'Switching decoder to two layer FC model with Tanh activations for PINO'
            )
        self.decoder_net = ConvFullyConnectedArch(input_keys=self.
            decoder_net.input_keys, output_keys=self.decoder_net.
            output_keys, layer_size=self.fno_layer_size, nr_layers=1,
            activation_fn=Activation.TANH, skip_connections=False,
        ...
        self.output_key_fno_dict = {str(var): var.size for var in self.
            output_keys_fno}
    ...
    @torch.jit.ignore
    def calc_pino_derivatives(self, latent: paddle.Tensor) ->Dict[str, Tensor]:
        ...
        if self.first_order_pino:
            output_dx = first_order_pino_grads(u=latent, ux=lat_dx,
                weights_1=weights[0], weights_2=weights[1], bias_1=biases[0])
            dims = ['x', 'y', 'z']
            for d in range(len(output_dx)):
                for k, v in zip(self.output_keys_fno, paddle.split(x=
                    output_dx[d], num_or_sections=output_dx[d].shape[1] //
                    list(self.output_key_fno_dict.values()), axis=1)):
                    if (f'{k}__{dims[d]}__{dims[d]}' in self.
                        output_key_dict.keys()):
                        out_scale = self.decoder_net.output_scales[str(k)][1]
                        outputs[f'{k}__{dims[d]}'] = v * out_scale
        if self.second_order_pino:
            output_dxx = second_order_pino_grads(u=latent, ux=lat_dx, uxx=
                lat_ddx, weights_1=weights[0], weights_2=weights[1], bias_1
                =biases[0])
            dims = ['x', 'y', 'z']
            for d in range(len(output_dxx)):
                for k, v in zip(self.output_keys_fno, paddle.split(x=
                    output_dxx[d], num_or_sections=output_dxx[d].shape[1] //
                    list(self.output_key_fno_dict.values()), axis=1)):
                    if (f'{k}__{dims[d]}__{dims[d]}' in self.
                        output_key_dict.keys()):
                        out_scale = self.decoder_net.output_scales[str(k)][1]
