...
class RadialBasisArch(Arch):
    ...
    def __init__(self, input_keys: List[Key], output_keys: List[Key],
        bounds: Dict[str, List[float]], detach_keys: List[Key]=[],
        nr_centers: int=128, sigma: float=0.1) ->None:
        super().__init__(input_keys=input_keys, output_keys=output_keys,
            detach_keys=detach_keys)
        out_features = sum(self.output_key_dict.values())
        self.nr_centers = nr_centers
        self.sigma = sigma
        out_55 = paddle.create_parameter(shape=paddle.empty(shape=[
            nr_centers, len(bounds)]).shape, dtype=paddle.empty(shape=[
            nr_centers, len(bounds)]).numpy().dtype, default_initializer=
            paddle.nn.initializer.Assign(paddle.empty(shape=[nr_centers,
            len(bounds)])))
        out_55.stop_gradient = not False
        self.centers = out_55
        with paddle.no_grad():
            for idx, bound in enumerate(bounds.values()):
                self.centers[:, idx].uniform_(min=bound[0], max=bound[1])
        self.fc_layer = layers.FCLayer(nr_centers, out_features,
    ...
    def _tensor_forward(self, x: paddle.Tensor) ->paddle.Tensor:
        ...
        radial_activation = paddle.exp(x=-0.5 * paddle.square(x=paddle.
            linalg.norm(x=centers - x, p=2, axis=-1) / self.sigma))
    ...
    def _dict_forward(self, in_vars: Dict[str, Tensor]) ->Dict[str, Tensor]:
        ...
        """
        x = self.prepare_input(in_vars, self.input_key_dict.keys(), self.
            detach_key_dict, -1)
        shape = x.shape[0], self.nr_centers, x.shape[1]
        x = x.unsqueeze(axis=1).expand(shape=shape)
        centers = self.centers.expand(shape=shape)
        radial_activation = paddle.exp(x=-0.5 * paddle.square(x=paddle.
            linalg.norm(x=centers - x, p=2, axis=-1) / self.sigma))
        x = self.fc_layer(radial_activation)
