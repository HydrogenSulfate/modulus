...
class DistributedManager(object):
    ...
    def __new__(cls):
        ...
        if not hasattr(obj, '_device'):
            obj._device = str(f'cuda:0' if paddle.device.cuda.device_count(
                ) >= 1 else 'cpu').replace('cuda', 'gpu')
    ...
    def get_available_backend():
        if paddle.device.cuda.device_count(
            ) >= 1 and paddle.distributed.is_nccl_available():
            return 'nccl'
        else:
            return 'gloo'
    ...
    def initialize_env():
        ...
        DistributedManager.setup(rank=rank, world_size=world_size,
            local_rank=local_rank, addr=addr, port=port, backend=
            DistributedManager.get_available_backend())
    ...
    def initialize_open_mpi(addr, port):
        ...
        DistributedManager.setup(rank=rank, world_size=world_size,
            local_rank=local_rank, addr=addr, port=port, backend=
            DistributedManager.get_available_backend(), method='openmpi')
    ...
    def initialize_slurm(port):
        ...
        DistributedManager.setup(rank=rank, world_size=world_size,
            local_rank=local_rank, addr=addr, port=port, backend=
            DistributedManager.get_available_backend(), method='slurm')
    ...
    @staticmethod
    def setup(rank=0, world_size=1, local_rank=None, addr='localhost', port
        ='12355', backend='nccl', method='env'):
        os.environ['MASTER_ADDR'] = addr
        os.environ['MASTER_PORT'] = str(port)
        manager = DistributedManager()
        manager._distributed = (world_size > 1 and paddle.distributed.
            is_available())
        ...
        manager._device = str(f'cuda:{manager.local_rank}' if paddle.device
            .cuda.device_count() >= 1 else 'cpu').replace('cuda', 'gpu')
        if paddle.device.cuda.device_count() >= 1:
            paddle.device.set_device(device=manager.local_rank)
        manager._initialization_method = method
        paddle.device.set_device(manager.place)
        paddle.device.cuda.empty_cache()

    @staticmethod
    def create_process_subgroup(name: str, size: int, group_name=None,
        verbose=False):
        manager = DistributedManager()
        ...
        group_size = paddle.distributed.get_world_size(group=group)
        group_rank = paddle.distributed.get_rank(group=group)
        num_groups = manager.world_size // group_size
        assert group_size % size == 0, f'Cannot divide group size {group_size} evenly into subgroups of size {size}'
        ...
        for g in range(num_groups):
            for i in range(num_subgroups):
                ...
                tmp_group = paddle.distributed.new_group(ranks=ranks)
    ...
    @staticmethod
    def create_orthogonal_process_group(name: str, group_name: str, verbose
        =False):
        manager = DistributedManager()
        if not manager.distributed:
            return None
        assert group_name in manager._groups, f'Group with name {group_name} does not exist'
        assert name not in manager._groups, f'Group with name {name} already exists'
