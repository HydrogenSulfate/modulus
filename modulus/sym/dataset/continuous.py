...
class DictPointwiseDataset(_DictPointwiseDatasetMixin, Dataset):
    ...
    def __init__(self, invar: Dict[str, np.array], outvar: Dict[str, np.
        array], lambda_weighting: Dict[str, np.array]=None):
        super().__init__(invar=invar, outvar=outvar, lambda_weighting=
            lambda_weighting)

    def __getitem__(self, idx):
        invar = _DictDatasetMixin._idx_var(self.invar, idx)
        outvar = _DictDatasetMixin._idx_var(self.outvar, idx)
        lambda_weighting = _DictDatasetMixin._idx_var(self.lambda_weighting,
            idx)
...
class DictInferencePointwiseDataset(Dataset):
    ...
    def __init__(self, invar: Dict[str, np.array], output_names: List[str]):
...
class ContinuousPointwiseIterableDataset(IterableDataset):
    ...
    def __init__(self, invar_fn: Callable, outvar_fn: Callable,
        ...
        if lambda_weighting_fn is None:
            lambda_weighting_fn = lambda _, outvar: {key: np.ones_like(x) for
                key, x in outvar.items()}

        def iterable_function():
            while True:
                invar = Dataset._to_tensor_dict(self.invar_fn())
                outvar = Dataset._to_tensor_dict(self.outvar_fn(invar))
                lambda_weighting = Dataset._to_tensor_dict(self.
                    lambda_weighting_fn(invar, outvar))
...
class DictImportanceSampledPointwiseIterableDataset(_DictPointwiseDatasetMixin,
    IterableDataset):
    """
    ...
    def __init__(self, invar: Dict[str, np.array], outvar: Dict[str, np.
        array], batch_size: int, importance_measure: Callable,
        lambda_weighting: Dict[str, np.array]=None, shuffle: bool=True,
        ...
        def iterable_function():
            ...
            while True:
                if counter % self.resample_freq == 0:
                    list_importance = []
                    list_invar = {key: np.split(value, value.shape[0] //
                        self.batch_size) for key, value in self.invar.items()}
                    for i in range(len(next(iter(list_invar.values())))):
                        importance = self.importance_measure({key: value[i] for
                            key, value in list_invar.items()})
                        list_importance.append(importance)
                    importance = np.concatenate(list_importance, axis=0)
                    prob = importance / np.sum(self.invar['area'].numpy() *
                        importance)
                idx = np.array([])
                while True:
                ...
                invar = _DictDatasetMixin._idx_var(self.invar, idx)
                outvar = _DictDatasetMixin._idx_var(self.outvar, idx)
                lambda_weighting = _DictDatasetMixin._idx_var(self.
                    lambda_weighting, idx)
...
class ListIntegralDataset(_DictDatasetMixin, Dataset):
    ...
    def __init__(self, list_invar: List[Dict[str, np.array]], list_outvar:
        List[Dict[str, np.array]], list_lambda_weighting: List[Dict[str, np
        .array]]=None):
        if list_lambda_weighting is None:
            list_lambda_weighting = []
            for outvar in list_outvar:
                list_lambda_weighting.append({key: np.ones_like(x) for key,
                    x in outvar.items()})
    ...
    def save_dataset(self, filename):
        for idx in range(self.length):
            var_to_polyvtk(filename + '_' + str(idx).zfill(5),
...
class ContinuousIntegralIterableDataset(IterableDataset):
    ...
    def __init__(self, invar_fn: Callable, outvar_fn: Callable, batch_size:
        int, lambda_weighting_fn: Callable=None, param_ranges_fn: Callable=None
        ):
        self.invar_fn = invar_fn
        self.outvar_fn = outvar_fn
        self.lambda_weighting_fn = lambda_weighting_fn
        ...
        def iterable_function():
            while True:
                ...
                for _ in range(self.batch_size):
                    ...
                    if not param_range:
                        param_range = {'_': next(iter(list_invar[-1].values
                            ()))[0:1]}
                    list_outvar.append(self.outvar_fn(param_range))
                    list_lambda_weighting.append(self.lambda_weighting_fn(
                        param_range, list_outvar[-1]))
                invar = Dataset._to_tensor_dict(_stack_list_numpy_dict(
                    list_invar))
                outvar = Dataset._to_tensor_dict(_stack_list_numpy_dict(
                    list_outvar))
...
class DictVariationalDataset(Dataset):
    ...
    def __init__(self, invar: Dict[str, np.array], outvar_names: List[str]):
        self.invar = Dataset._to_tensor_dict(invar)
