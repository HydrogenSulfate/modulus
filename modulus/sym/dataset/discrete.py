...
class HDF5GridDataset(Dataset):
    ...
    def __init__(self, filename: Union[str, Path], invar_keys: List[str],
        outvar_keys: List[str], n_examples: int=None):
        ...
        if n_examples is not None:
            assert n_examples <= length, 'error, n_examples greater than length of file data'
            length = min(n_examples, length)
        self.length = length

    def __getitem__(self, idx):
        invar = Dataset._to_tensor_dict({k: self.f[k][idx, ...] for k in
            self.invar_keys})
        outvar = Dataset._to_tensor_dict({k: self.f[k][idx, ...] for k in
            self.outvar_keys})
        lambda_weighting = Dataset._to_tensor_dict({k: np.ones_like(v) for 
            k, v in outvar.items()})
        return invar, outvar, lambda_weighting
