...
class _BaseDataset:
    ...
    def worker_init_fn(self, iworker):
        ...
        worker_rank = manager.group_rank('data_parallel'
            ) if manager.distributed else 0
        worker_size = manager.group_size('data_parallel'
            ) if manager.distributed else 1
    ...
    def _to_tensor_dict(var_dict, device=None):
        tensor_dict = {key: paddle.to_tensor(data=value, place=device).
            astype(tf_dt) for key, value in var_dict.items()}
        return tensor_dict
...
class _DictDatasetMixin:
    ...
    def __init__(self, invar: Dict[str, np.array], outvar: Dict[str, np.
        ...
        if lambda_weighting is None:
            lambda_weighting = {key: np.ones_like(x) for key, x in outvar.
                items()}
