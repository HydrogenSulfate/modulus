...
class VoxelInferencer(PointVTKInferencer):
    ...
    def __init__(self, bounds: List[List[int]], npoints: List[int], nodes:
        List[Node], output_names: List[str], export_map: Union[None, Dict[
        str, List[str]]]=None, invar: Dict[str, np.array]={}, batch_size:
        int=1024, mask_fn: Union[Callable, None]=None, mask_value: float=np
        .nan, plotter=None, requires_grad: bool=False, log_iter: bool=False,
        model=None):
        ...
        vtk_obj = VTKUniformGrid(bounds=bounds, npoints=npoints, export_map
            =export_map)
        super().__init__(vtk_obj, nodes, input_vtk_map=input_vtk_map,
            output_names=output_names, invar=invar, batch_size=batch_size,
            mask_fn=mask_fn, mask_value=mask_value, plotter=plotter,
            requires_grad=requires_grad, log_iter=log_iter, model=model)
