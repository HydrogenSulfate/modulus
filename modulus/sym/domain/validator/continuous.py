...
class PointwiseValidator(Validator):
    ...
    def __init__(self, nodes: List[Node], invar: Dict[str, np.array],
        true_outvar: Dict[str, np.array], batch_size: int=1024, plotter:
        ValidatorPlotter=None, requires_grad: bool=False):
    ...
    def save_results(self, name, results_dir, writer, save_filetypes, step):
        ...
        losses = PointwiseValidator._l2_relative_error(true_outvar_cpu,
            pred_outvar_cpu)
        ...
        if 'vtk' in save_filetypes:
            var_to_polyvtk({**invar, **named_true_outvar, **
                named_pred_outvar}, results_dir + name)
...
class PointVTKValidator(PointwiseValidator):
    ...
    def __init__(self, vtk_obj: VTKBase, nodes: List[Node], input_vtk_map:
        Dict[str, List[str]], true_vtk_map: Dict[str, List[str]], invar:
        Dict[str, np.array]={}, true_outvar: Dict[str, np.array]={},
        batch_size: int=1024, plotter: ValidatorPlotter=None, requires_grad:
        bool=False, log_iter: bool=False):
        ...
        super().__init__(nodes=nodes, invar=invar, true_outvar=true_outvar,
            batch_size=batch_size, plotter=plotter, requires_grad=requires_grad
            )
