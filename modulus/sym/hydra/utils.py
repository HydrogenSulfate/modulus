...
def main(config_path: str, config_name: str='config'):
    ...
    def register_decorator(func):
        ...
        def func_decorated(cfg_passthrough: Optional[DictConfig]=None) ->Any:
            ...
            model_parallel_size = os.getenv('MODEL_PARALLEL_SIZE')
            if model_parallel_size:
                DistributedManager.create_process_subgroup('model_parallel',
                    int(model_parallel_size), verbose=True)
            ...
            if cfg_passthrough is not None:
                ...
                _run_hydra(args=args_parser.parse_args(), args_parser=
                    args_parser, task_function=func, config_path=
                    config_path, config_name=config_name)
        return func_decorated
    return register_decorator


def compose(config_name: Optional[str]=None, config_path: Optional[str]=
    None, overrides: List[str]=[], return_hydra_config: bool=False,
    job_name: Optional[str]='app', caller_stack_depth: int=2) ->DictConfig:
    ...
    hydra.initialize(config_path, job_name, caller_stack_depth)
    register_hydra_configs()
    register_callbacks_configs()
    register_loss_configs()
    register_metric_configs()
...
def instantiate_arch(cfg: ModelConf, input_keys: Union[List[Key], None]=
    None, output_keys: Union[List[Key], None]=None, detach_keys: Union[List
    [Key], None]=None, verbose: bool=False, **kwargs) ->Arch:
    assert hasattr(cfg, 'arch_type'
        ), 'Model configs are required to have an arch_type defined.         Improper architecture supplied, please make sure config         provided is a single arch config NOT the full hydra config!'
    try:
...
def instantiate_optim(cfg: DictConfig, model: paddle.nn.Layer, verbose:
    bool=False) ->paddle.optimizer.Optimizer:
    optim_cfg = copy.deepcopy(cfg.optimizer)
    del optim_cfg._params_
    try:
        optimizer = hydra.utils.instantiate(optim_cfg, parameters=model.
            parameters())
    except Exception as e:
...
def instantiate_sched(cfg: DictConfig, optimizer: paddle.optim
    ) ->paddle.optim.lr_scheduler:
    sched_cfg = copy.deepcopy(cfg.scheduler)
    if sched_cfg is MISSING:
        sched_cfg = {'_target_': 'paddle.optim.lr_scheduler.ConstantLR',
            'factor': 1.0}
    if sched_cfg._target_ == 'custom':
        if 'tf.ExponentialLR' in sched_cfg._name_:
            sched_cfg = {
                ...
                'learning_rate': sched_cfg.learning_rate
            }
        else:
...
def instantiate_agg(cfg: DictConfig, model: paddle.nn.Layer, num_losses: int=1
    ...
    try:
        aggregator = hydra.utils.instantiate(cfg.loss, model, num_losses,
            _convert_='all')
    except Exception as e:
