...
class AdvectionDiffusionConf(PDEConf):
    _target_: str = (
        'modulus.sym.eq.pdes.advection_diffusion.AdvectionDiffusion')
    T: str = 'T'
    D: str = 'D'
    Q: int = 0
    rho: str = 'rho'
...
class LinearElasticityPlaneConf(PDEConf):
    _target_: str = (
        'modulus.sym.eq.pdes.linear_elasticity.LinearElasticityPlaneStress')
    E = 10
    nu = 0.3
    lambda_ = None
    mu = None
...
def register_pde_configs() ->None:
    ...
    cs.store(group='pde', name='advection-diffusion', node=
        AdvectionDiffusionConf)
    cs.store(group='pde', name='diffusion', node=DiffusionConf)
    cs.store(group='pde', name='maxwell-real', node=MaxwellFreqRealConf)
    cs.store(group='pde', name='energy-fluid', node=EnergyFluidConf)
    cs.store(group='pde', name='linear-elasticity', node=LinearElasticityConf)
    cs.store(group='pde', name='linear-elasticity-plane', node=
        LinearElasticityPlaneConf)
    cs.store(group='pde', name='navier-stokes', node=NavierStokesConf)
    cs.store(group='pde', name='zero-eq-turbulence', node=ZeroEquationConf)
    cs.store(group='pde', name='wave', node=WaveEquationConf)
    cs.store(group='pde', name='helmholtz', node=HelmholtzEquationConf)
