...
class Solver(Trainer):
    ...
    def __init__(self, cfg: DictConfig, domain: Domain):
        ...
        if cfg.training.ntk.use_ntk:
            ntk = NTK(run_per_step=cfg.training.ntk.run_freq, save_name=cfg
                .training.ntk.save_name)
    ...
    def load_network(self):
        return Trainer._load_network(self.initialization_network_dir, self.
            network_dir, self.saveable_models, self.optimizer, self.
            aggregator, self.scheduler, self.scaler, self.log, self.manager,
            self.place)

    def load_optimizer(self):
        return Trainer._load_optimizer(self.network_dir, self.optimizer,
            self.aggregator, self.scheduler, self.scaler, self.log, self.place)

    def load_model(self):
        return Trainer._load_model(self.initialization_network_dir, self.
            network_dir, self.saveable_models, self.step, self.log, self.place)

    def load_step(self):
        return Trainer._load_step(self.network_dir, self.place)

    def save_checkpoint(self, step: int):
        Trainer._save_checkpoint(self.network_dir, self.saveable_models,
            self.optimizer, self.aggregator, self.scheduler, self.scaler, step)
    ...
    def record_inferencers(self, step: int):
        self.domain.rec_inferencers(self.network_dir, self.writer, self.
            save_filetypes, step)

    def record_stream(self, inferencer, name):
        return self.domain.rec_stream(inferencer, name, self.network_dir,
            self.step, self.save_results, self.save_filetypes, self.to_cpu)
