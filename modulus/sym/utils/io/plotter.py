...
class _Plotter:
    ...
    def _add_figures(self, group, name, results_dir, writer, step, *args):
        ...
        try:
            ...
            for f, tag in fs:
                f.savefig(results_dir + name + '_' + tag + '.png',
                    bbox_inches='tight', pad_inches=0.1)
                writer.add_figure(group + '/' + name + '/' + tag, f, step,
                    close=True)
            plt.close('all')
    ...
    def _interpolate_2D(self, size, invar, *outvars):
        ...
        xyi = np.meshgrid(np.linspace(extent[0], extent[1], size), np.
            linspace(extent[2], extent[3], size), indexing='ij')
        outvars_interp = []
        for outvar in outvars:
            outvar_interp = {}
            for k in outvar:
                outvar_interp[k] = scipy.interpolate.griddata((xs[0], xs[1]
                    ), outvar[k][:, 0], tuple(xyi))
...
class ValidatorPlotter(_Plotter):
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        if ndim > 2:
            ...
            return []
        if ndim == 2:
            extent, true_outvar, pred_outvar = self._interpolate_2D(100,
                invar, true_outvar, pred_outvar)
        dims = list(invar.keys())
        fs = []
        for k in pred_outvar:
            f = plt.figure(figsize=(3 * 5, 4), dpi=100)
            for i, (o, tag) in enumerate(zip([true_outvar[k], pred_outvar[k
                ], true_outvar[k] - pred_outvar[k]], ['true', 'pred', 'diff'])
                ):
                plt.subplot(1, 3, 1 + i)
                if ndim == 1:
...
class GridValidatorPlotter(_Plotter):
    ...
    def __call__(self, invar: Dict[str, np.array], true_outvar: Dict[str,
        np.array], pred_outvar: Dict[str, np.array]):
        ndim = next(iter(invar.values())).ndim - 2
        if ndim > 3:
            print(
                'Default plotter can only handle <=3 input dimensions, passing'
                )
            return []
        diff_outvar = {}
        ...
        for ie in range(self.n_examples):
            f = self._make_plot(ndim, ie, invar, true_outvar, pred_outvar,
                diff_outvar)
    ...
    def _make_plot(self, ndim, ie, invar, true_outvar, pred_outvar, diff_outvar
        ...
        for ic, (d, tag) in enumerate(zip([invar, true_outvar, pred_outvar,
            diff_outvar], ['in', 'true', 'pred', 'diff'])):
...
class DeepONetValidatorPlotter(_Plotter):
    ...
    def _make_plot(self, ndim, ie, invar, true_outvar, pred_outvar, diff_outvar
        ...
        for ic, (d, tag) in enumerate(zip([true_outvar, pred_outvar,
            diff_outvar], ['true', 'pred', 'diff'])):
            for ir, k in enumerate(d):
                plt.subplot2grid((nrows, 4), (ir, ic))
                if ndim == 1:
                    plt.plot(invar_data[ie, :].flatten(), d[k][ie, :])
                elif ndim == 2:
                    plt.scatter(x=invar_data[ie, :, 0], y=invar_data[ie, :,
                        1], c=d[k][ie, :], s=0.5, origin='lower', cmap='jet')
                    plt.colorbar()
                plt.title(f'{k}_{tag}')
        plt.tight_layout()
        return f
