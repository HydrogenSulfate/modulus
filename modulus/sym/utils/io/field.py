...
def plot_field(var, save_name, coordinates=['x', 'y'], bounds_var=None,
    criteria=None, plot_title='', resolution=128, figsize=(8, 8)):
    plt.figure(figsize=figsize)
    nr_plots = len(list(var.keys())) - 2
    plot_index = 1
    for key, value in var.items():
        ...
        value_star = interpolate.griddata(pos, value.flatten(), (X, Y),
            method='linear')
        ...
        plt.imshow(np.flip(value_star, axis=0), cmap='jet', extent=[np.min(
...
def plot_field_compare(true_var, pred_var, save_name, coordinates=['x', 'y'
    ], bounds_var=None, criteria=None, resolution=128, figsize=(12, 10),
    same_colorbar=False):
    plt.figure(figsize=figsize)
    nr_plots = len(list(true_var.keys())) - 2
    plot_index = 1
    for key in true_var.keys():
        if key in coordinates:
            continue
        X, Y = _make_mesh(true_var, coordinates, bounds_var, resolution)
        pos = np.concatenate([true_var[coordinates[0]], true_var[
            coordinates[1]]], axis=1)
        true_field_star = interpolate.griddata(pos, true_var[key].flatten(),
            (X, Y), method='linear')
        pred_field_star = interpolate.griddata(pos, pred_var[key].flatten(),
            (X, Y), method='linear')
        if criteria is not None:
            np_criteria = _compile_criteria(coordinates, criteria)
            nan_mask = np.where(np_criteria(X, Y), 0.0, np.nan)
...
def _make_mesh(var, coordinates, bounds_var, resolution):
    ...
    if bounds_var is not None:
        x = np.linspace(bounds_var[coordinates[0] + '_min'], bounds_var[
            coordinates[0] + '_max'], nn_x)
        y = np.linspace(bounds_var[coordinates[1] + '_min'], bounds_var[
            coordinates[1] + '_max'], nn_y)
    else:
        x = np.linspace(np.min(var[coordinates[0]]), np.max(var[coordinates
            [0]]), nn_x)
        y = np.linspace(np.min(var[coordinates[1]]), np.max(var[coordinates
            [1]]), nn_y)
    return np.meshgrid(x, y)


def _compile_criteria(coordinates, criteria):
    np_criteria = sp.lambdify([sp.Symbol(k) for k in coordinates], criteria,
        'numpy')
    return np_criteria


def _var_to_mesh(var, key, coordinates=['x', 'y'], bounds_var=None,
    criteria=None, resolution=128):
    X, Y = _make_mesh(var, coordinates, bounds_var, resolution)
    pos = np.concatenate([var[coordinates[0]], var[coordinates[1]]], axis=1)
    value_star = interpolate.griddata(pos, var[key].flatten(), (X, Y),
        method='linear')
    if criteria is not None:
        np_criteria = _compile_criteria(coordinates, criteria)
        nan_mask = np.where(np_criteria(X, Y), 0.0, np.nan)
