...
class VTKBase:
            ...
            for vtk_key in vtk_keys:
                if vtk_key in coord_map:
                    ...
                    if len(vtk_key.split(':')) > 1:
                        input_array0 = self.get_array(name=vtk_key.split(
                            ':')[0], dim=int(vtk_key.split(':')[1]))
                    else:
    ...
    def var_to_vtk(self, data_vars: Dict[str, np.array], file_name: str=
        None, file_dir: str=None, step: int=None):
        ...
        for key, data_keys in self.export_map.items():
            ...
            for data_key in data_keys:
                if data_key in data_vars:
                    ...
                    vtk_array.append(np.zeros((self.vtk_obj.
                        GetNumberOfPoints(), 1), dtype=np.short))
            if len(vtk_array) > 0:
    ...
    def save_vtk(self, file_name: str=None, file_dir: str=None, compression:
        int=1, data_mode: int=1):
        if file_name is None:
            file_name = self.file_name
        if file_dir is None:
            file_dir = self.file_dir
...
class VTKUniformGrid(VTKBase):
    ...
    def __init__(self, bounds: List[List[int]], npoints: List[int],
        export_map: Dict[str, List[str]]={}, file_name: str='vtk_output',
        file_dir: str='.', init_vtk: bool=True):
    ...
    def init_points(self, bounds: List[List[int]], npoints: List[int]):
        assert len(bounds) == len(npoints
            ), f'Bounds and npoints must be same length {len(bounds)}, {len(npoints)}'
        assert len(bounds) > 0 and len(bounds
            ) < 4, 'Only 1, 2, 3 grid dimensionality allowed'
        npoints = np.array(npoints + [1, 1])
        bounds = np.array(bounds + [[0, 0], [0, 0]])
        dx = abs(bounds[:, 0] - bounds[:, 1]) / np.maximum(np.ones_like(
            npoints), npoints - 1)
        self.vtk_obj.SetOrigin(bounds[0][0], bounds[1][0], bounds[2][0])
    ...
    @classmethod
    def init_from_obj(cls, vtk_obj, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.'):
        vtk_wrapper = VTKUniformGrid(None, None, export_map=export_map,
            file_name=file_name, file_dir=file_dir, init_vtk=False)
...
class VTKRectilinearGrid(VTKBase):
    ...
    def __init__(self, axis_coords: List[np.array], export_map: Dict[str,
        List[str]]={}, file_name: str='vtk_output', file_dir: str='.',
        init_vtk: bool=True):
        super().__init__(file_name, file_dir)
    ...
    def init_points(self, coords: List[np.array]):
        assert len(coords
            ) < 4, 'Maximum  of 3 spacial coordinate arrays accepted'
        coords = coords + [np.array([0]), np.array([0])]
        self.vtk_obj.SetDimensions(coords[0].shape[0], coords[1].shape[0],
            coords[2].shape[0])
    ...
    def set_points(self, points: np.array):
        raise AttributeError(
            'Cannot set the points of a vtkRectilinearGrid explicitly')

    def set_cells(self):
        raise AttributeError(
            'Cannot set the cells of a vtkRectilinearGrid explicitly')

    @classmethod
    def init_from_obj(cls, vtk_obj, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.'):
        vtk_wrapper = VTKRectilinearGrid(None, export_map=export_map,
            file_name=file_name, file_dir=file_dir, init_vtk=False)
        vtk_wrapper.vtk_obj = vtk_obj
        return vtk_wrapper
...
class VTKStructuredGrid(VTKBase):
    ...
    def __init__(self, points: np.array, dims: List[int], export_map: Dict[
        str, List[str]]={}, file_name: str='vtk_output', file_dir: str='.',
        init_vtk: bool=True):
        super().__init__(file_name, file_dir)
    ...
    def init_points(self, points: np.array, dims: List[int]):
        ...
        assert points.shape[1
            ] < 4, 'Maximum  of 3 spacial point arrays accepted'
        assert len(dims) == points.shape[1
            ], 'Domain dimension must match dim of points'
        points = np.concatenate([points, np.zeros((points.shape[0], 2),
            dtype=np.short)], axis=1)
        dims = dims + [1, 1]
    ...
    def set_cells(self):
        raise AttributeError(
            'Cannot set the cells of a vtkStructuredGrid explicitly')

    @classmethod
    def init_from_obj(cls, vtk_obj, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.'):
        vtk_wrapper = VTKStructuredGrid(None, None, export_map=export_map,
            file_name=file_name, file_dir=file_dir, init_vtk=False)
        vtk_wrapper.vtk_obj = vtk_obj
        return vtk_wrapper
...
class VTKUnstructuredGrid(VTKBase):
    ...
    def __init__(self, points: np.array, cell_index: Tuple[np.array, np.
        array], cell_types: np.array, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.', init_vtk: bool=True):
        super().__init__(file_name, file_dir)
    ...
    def init_points(self, points: np.array, cell_index: Tuple[np.array, np.
        array], cell_types: np.array):
        assert points.ndim == 2, 'Points array must have 2 dimensions [npoints, dim]'
        assert points.shape[1
            ] < 4, 'Maximum  of 3 spacial point arrays accepted'
        assert len(cell_index
            ) == 2, 'Cell index must be tuple of numpy arrays containing [offsets, connectivity]'
        points = np.concatenate([points, np.zeros((points.shape[0], 2),
            dtype=np.short)], axis=1)
        pts = vtk.vtkPoints()
        pts.SetNumberOfPoints(points.shape[0])
        ...
        vtk_celltypes = numpy_to_vtk(cell_types.astype(int), array_type=vtk
            .vtkUnsignedCharArray().GetDataType())
        vtk_cells = vtk.vtkCellArray()
        vtk_offsets = numpy_to_vtk(cell_index[0], array_type=vtk.
            vtkTypeInt64Array().GetDataType())
        vtk_connectivity = numpy_to_vtk(cell_index[1], array_type=vtk.
            vtkTypeInt64Array().GetDataType())
    ...
    def set_cells(self, cell_index: Tuple[np.array, np.array], cell_types:
        np.array):
        assert len(cell_index
            ) == 2, 'Cell index must be tuple of numpy arrays containing [offsets, connectivity]'
        vtk_celltypes = vtk.vtkIntArray()
    ...
    def init_from_obj(cls, vtk_obj, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.'):
        vtk_wrapper = VTKUnstructuredGrid(None, None, None, export_map=
            export_map, file_name=file_name, file_dir=file_dir, init_vtk=False)
        vtk_wrapper.vtk_obj = vtk_obj
        return vtk_wrapper
...
class VTKPolyData(VTKBase):
    ...
    def __init__(self, points: np.array, line_index: np.array=None,
        poly_index: Tuple[np.array, np.array]=None, export_map: Dict[str,
        List[str]]={}, file_name: str='vtk_output', file_dir: str='.',
        init_vtk: bool=True):
        super().__init__(file_name, file_dir)
    ...
    def init_points(self, points: np.array, line_index: np.array=None,
    ...
    def get_cells(self):
        raise AttributeError(
            'vtkPolyData has polys not cells, call get_polys instead')
    ...
    def set_lines(self, edge_index: np.array):
        assert edge_index.ndim == 2 and edge_index.shape[1
            ] == 2, 'Edge index array must have 2 dimensions [npoints, 2]'
    ...
    def set_polys(self, poly_index: Tuple[np.array, np.array]):
        assert len(poly_index
            ) == 2, 'poly_index should be tuple of (poly_offsets, poly_connectivity)'
        vtk_polys = vtk.vtkCellArray()
        vtk_offsets = numpy_to_vtk(poly_index[0], array_type=vtk.
            vtkTypeInt64Array().GetDataType())
        vtk_connectivity = numpy_to_vtk(poly_index[1], array_type=vtk.
            vtkTypeInt64Array().GetDataType())
    ...
    def set_cells(self):
        raise AttributeError(
            'vtkPolyData has polys not cells, call set_polys instead')

    @classmethod
    def init_from_obj(cls, vtk_obj, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.'):
        vtk_wrapper = VTKPolyData(None, export_map=export_map, file_name=
            file_name, file_dir=file_dir, init_vtk=False)
        vtk_wrapper.vtk_obj = vtk_obj
        return vtk_wrapper
...
class VTKFromFile(object):
    ...
    def __new__(cls, file_path: str, export_map: Dict[str, List[str]]={},
        file_name: str='vtk_output', file_dir: str='.', force_legacy: bool=
        False) ->None:
        assert Path(file_path).is_file(
            ), f'Provided VTK file {file_path} not found'
        read_success = False
        if not force_legacy:
            try:
        ...
        return cls.extractVTKObject(vtk_reader=vtk_reader, export_map=
            export_map, file_name=file_name, file_dir=file_dir)
...
def var_to_polyvtk(var_dict: Dict[str, np.array], file_path: str,
    coordinates=['x', 'y', 'z']):
    ...
    for key in var_dict.keys():
        export_map[key] = [key]
    file_path = Path(file_path)
    vtk_obj = VTKPolyData(points=points, export_map=export_map, file_name=
        file_path.stem, file_dir=file_path.parents[0])
    vtk_obj.var_to_vtk(data_vars=var_dict)


def grid_to_vtk(var_dict: Dict[str, np.array], file_path: str, batch_index:
    int=0):
    ...
    assert len(shape) > 2 and len(shape
        ) < 6, 'Input variables must be dim 3, 4, 5'
    bsize = shape[0]
    cdim = shape[1]
    ...
    for key in shaped_dict.keys():
        export_map[key] = [key]
    file_path = Path(file_path)
    vtk_obj = VTKUniformGrid(bounds=bounds, npoints=grid_shape, export_map=
        export_map, file_name=file_path.stem, file_dir=file_path.parents[0])
