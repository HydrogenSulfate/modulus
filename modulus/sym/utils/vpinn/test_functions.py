...
class Test_Function:
    ...
    def __init__(self, name_ord_dict=None, box=None, diff_list=None,
        weight_fcn=None, simplify=None):
    ...
    def generator(self, test_class):
        ...
        if self.dim == 1:
            x_trans = test_class.interval1d[0] + (test_class.interval1d[1] -
                test_class.interval1d[0]) / (self.ub[0] - self.lb[0]) * (x -
                self.lb[0])
            for k in ord_list:
                if test_class.is_real:
                    yield self.simplify_fcn(self.weight_fcn * test_class.
                        sympy_fcn(k, x_trans))
                else:
            ...
            x_trans = test_class.interval1d[0] + (test_class.interval1d[1] -
                test_class.interval1d[0]) / (self.ub[0] - self.lb[0]) * (x -
                self.lb[0])
            y_trans = test_class.interval1d[0] + (test_class.interval1d[1] -
                test_class.interval1d[0]) / (self.ub[1] - self.lb[1]) * (y -
                self.lb[1])
            ev = itertools.islice(Degree_nk(self.dim), ord_list[0], 
                ord_list[-1] + 1)
            for _ in ord_list:
                ord = next(ev)
                for k in ord:
                    if test_class.is_real:
                        yield self.simplify_fcn(self.weight_fcn *
                            test_class.sympy_fcn(k[0], x_trans) *
                            test_class.sympy_fcn(k[1], y_trans))
                    else:
                        for fx in test_class.sympy_fcn(k[0], x_trans
                            ).as_real_imag():
                            for fy in test_class.sympy_fcn(k[1], y_trans
                                ).as_real_imag():
                                yield self.simplify_fcn(self.weight_fcn *
                                    fx * fy)
        else:
            ...
            y_trans = test_class.interval1d[0] + (test_class.interval1d[1] -
                test_class.interval1d[0]) / (self.ub[1] - self.lb[1]) * (y -
                self.lb[1])
            z_trans = test_class.interval1d[0] + (test_class.interval1d[1] -
                test_class.interval1d[0]) / (self.ub[2] - self.lb[2]) * (z -
                self.lb[2])
            ev = itertools.islice(Degree_nk(self.dim), ord_list[0], 
                ord_list[-1] + 1)
            for _ in ord_list:
                    ...
                    if test_class.is_real:
                        ...
                        for fx in test_class.sympy_fcn(k[0], x_trans
                            ...
                            for fy in test_class.sympy_fcn(k[1], y_trans
                                ...
                                for fz in test_class.sympy_fcn(k[2], z_trans
                                    ).as_real_imag():
                                    yield self.simplify_fcn(self.weight_fcn *
                                        fx * fy * fz)
        return
        ...
        if self.dim == 1:
            for name in self.name_ord_dict.keys():
                for fcn in self.generator(name):
                    ...
                    for k in self.diff_list:
                        if k == 'grad':
                            self.test_sympy_dict['vx'].append(self.
                                simplify_fcn(sp.diff(fcn, x)))
                        elif k == 'Delta':
                            self.test_sympy_dict['dv'].append(self.
                                simplify_fcn(sp.diff(fcn, x, 2)))
                        else:
                            self.test_sympy_dict['v' + 'x' * k[0]].append(self
                                .simplify_fcn(sp.diff(fcn, x, k[0])))
        elif self.dim == 2:
            for name in self.name_ord_dict.keys():
                for fcn in self.generator(name):
                    self.test_sympy_dict['v'].append(fcn)
                    for k in self.diff_list:
                        if k == 'grad':
                            self.test_sympy_dict['vx'].append(self.
                                simplify_fcn(sp.diff(fcn, x)))
                            self.test_sympy_dict['vy'].append(self.
                                simplify_fcn(sp.diff(fcn, y)))
                        elif k == 'Delta':
                            self.test_sympy_dict['dv'].append(self.
                                simplify_fcn(sp.diff(fcn, x, 2) + sp.diff(
                                fcn, y, 2)))
                        else:
                            self.test_sympy_dict['v' + 'x' * k[0] + 'y' * k[1]
                                ].append(self.simplify_fcn(sp.diff(fcn, x,
                                k[0], y, k[1])))
        elif self.dim == 3:
            for name in self.name_ord_dict.keys():
                for fcn in self.generator(name):
                    ...
                    for k in self.diff_list:
                        if k == 'grad':
                            ...
                            self.test_sympy_dict['vy'].append(self.
                                simplify_fcn(sp.diff(fcn, y)))
                            self.test_sympy_dict['vz'].append(self.
                                simplify_fcn(sp.diff(fcn, z)))
                        elif k == 'Delta':
                            self.test_sympy_dict['dv'].append(self.
                                simplify_fcn(sp.diff(fcn, x, 2) + sp.diff(
                                fcn, y, 2) + sp.diff(fcn, z, 2)))
                        else:
                            self.test_sympy_dict['v' + 'x' * k[0] + 'y' * k
                                [1] + 'z' * k[2]].append(self.simplify_fcn(
    ...
    def lambdify(f_sympy, var_list):
        ...
        if f_sympy.is_number:
            if dim == 1:
                return lambda x0, f_sympy=f_sympy: paddle.zeros_like(x=x0
                    ) + float(f_sympy)
            elif dim == 2:
                return lambda x0, y0, f_sympy=f_sympy: paddle.zeros_like(x=x0
                    ) + float(f_sympy)
            elif dim == 3:
                return lambda x0, y0, z0, f_sympy=f_sympy: paddle.zeros_like(x
                    =x0) + float(f_sympy)
        else:
    ...
    def lambdify_fcn_list(self):
        ...
        for k in self.test_sympy_dict.keys():
            ...
            for f_sympy in self.test_sympy_dict[k]:
                self.test_lambda_dict[k].append(Test_Function.lambdify(
                    f_sympy, var_list))
    ...
    def eval_test(self, ind, x, y=None, z=None):
        ...
        for f in self.test_lambda_dict[ind]:
            if self.dim == 1:
                ...
                assert y is not None and z is not None, 'please provide tensor y and z'
                tmp_list.append(f(x, y, z))
        return paddle.concat(x=tmp_list, axis=1)
...
class Vector_Test:
    ...
    def sample_vector_test(self):
        ...
        if mix is None or mix == 'all' or mix == 1:
            ...
            if self.num == 2:
                self.output_ind = [k for k in itertools.product(range(self.
                    v1.num_fcn), range(self.v2.num_fcn))]
            else:
                self.output_ind = [k for k in itertools.product(range(self.
                    v1.num_fcn), range(self.v2.num_fcn), range(self.v3.
                    num_fcn))]
        elif 0 < mix < 1:
            self.mix = mix
            self.num_output = int(self.mix * self.num_fcn
                ) if self.mix > 0 else 1
            if self.num == 2:
                self.output_ind = random.sample(set(itertools.product(range
                    (self.v1.num_fcn), range(self.v2.num_fcn))), self.
                    num_output)
            else:
                self.output_ind = random.sample(set(itertools.product(range
                    (self.v1.num_fcn), range(self.v2.num_fcn), range(self.
                    v3.num_fcn))), self.num_output)
        elif mix >= 1:
            self.mix = int(mix)
    ...
    def eval_test(self, ind, x, y=None, z=None):
        ...
        if self.num == 2:
            x_ind = paddle.to_tensor(data=[k[0] for k in self.output_ind],
                place=x.place)
            y_ind = paddle.to_tensor(data=[k[1] for k in self.output_ind],
                place=x.place)
            return v1_val.index_select(axis=1, index=x_ind
                ), v2_val.index_select(axis=1, index=y_ind)
        else:
            ...
            y_ind = paddle.to_tensor(data=[k[1] for k in self.output_ind],
                place=x.place)
            z_ind = paddle.to_tensor(data=[k[2] for k in self.output_ind],
                place=x.place)
            return v1_val.index_select(axis=1, index=x_ind
                ), v2_val.index_select(axis=1, index=y_ind
                ), v3_val.index_select(axis=1, index=z_ind)


class RBF_Function:

    def __init__(self, dim=2, RBF_name=None, diff_list=None, weight_fcn=
        None, simplify=None):
        if RBF_name is None:
    ...
    def make_fcn_list(self):
        ...
        if self.dim == 1:
            for k in self.diff_list:
                if k == 'grad':
                    self.test_sympy_dict['vx'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, x))
                elif k == 'Delta':
                    self.test_sympy_dict['dv'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, x, 2))
                else:
            ...
            for k in self.diff_list:
                if k == 'grad':
                    self.test_sympy_dict['vx'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, x))
                    self.test_sympy_dict['vy'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, y))
                elif k == 'Delta':
                    self.test_sympy_dict['dv'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, x, 2) + sp.diff(self.
                        RBF_prototype, y, 2))
                else:
                    self.test_sympy_dict['v' + 'x' * k[0] + 'y' * k[1]
                        ] = self.simplify_fcn(sp.diff(self.RBF_prototype, x,
                        k[0], y, k[1]))
        else:
            for k in self.diff_list:
                if k == 'grad':
                    ...
                    self.test_sympy_dict['vy'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, y))
                    self.test_sympy_dict['vz'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, z))
                elif k == 'Delta':
                    self.test_sympy_dict['dv'] = self.simplify_fcn(sp.diff(
                        self.RBF_prototype, x, 2) + sp.diff(self.
                        RBF_prototype, y, 2) + sp.diff(self.RBF_prototype,
                        z, 2))
                else:
                    self.test_sympy_dict['v' + 'x' * k[0] + 'y' * k[1] + 
                        'z' * k[2]] = self.simplify_fcn(sp.diff(self.
                        RBF_prototype, x, k[0], y, k[1], z, k[2]))
    ...
    def lambdify_fcn_list(self):
        ...
        for k in self.test_sympy_dict.keys():
            ...
            self.test_lambda_dict[k] = torch_lambdify(f_sympy, var_list,
                separable=True)

    def eval_test(self, ind, x, y=None, z=None, x_center=None, y_center=
        None, z_center=None, eps=None):
        assert x_center is not None, 'please provide x_center'
        if eps is None:
            eps = paddle.full(shape=[1, x_center.shape[0]], fill_value=10.0)
        elif isinstance(eps, int) or isinstance(eps, float):
            eps = paddle.full(shape=[1, x_center.shape[0]], fill_value=np.
                float32(eps))
        elif isinstance(eps, paddle.Tensor):
            eps = paddle.reshape(x=eps, shape=[1, -1])
        x = x_center
        perm_0 = list(range(x.ndim))
        perm_0[0] = 1
        perm_0[1] = 0
        x_center_t = paddle.transpose(x=x, perm=perm_0)
        if self.dim == 1:
            ...
            perm_1[0] = 1
            perm_1[1] = 0
            y_center_t = paddle.transpose(x=x, perm=perm_1)
            x_new = eps * (x - x_center_t)
            ...
            perm_2[0] = 1
            perm_2[1] = 0
            y_center_t = paddle.transpose(x=x, perm=perm_2)
            x = z_center
            perm_3 = list(range(x.ndim))
            perm_3[0] = 1
            perm_3[1] = 0
            z_center_t = paddle.transpose(x=x, perm=perm_3)
            x_new = eps * (x - x_center_t)
            y_new = eps * (y - y_center_t)
        ...
        p = self.pow_dict[ind]
        if self.dim == 1:
            return fcn(x_new) * paddle.pow(x=eps, y=p)
        elif self.dim == 2:
            return fcn(x_new, y_new) * paddle.pow(x=eps, y=p)
        else:
            return fcn(x_new, y_new, z_new) * paddle.pow(x=eps, y=p)
