...
class Quadrature:
    ...
    def make_tensor(self):
        self.points_tensor = paddle.to_tensor(data=self.points_numpy, dtype
            ='float32')
        self.weights_tensor = paddle.to_tensor(data=self.weights_numpy.
            reshape((-1, 1)), dtype='float32')
...
class Quad_Line(Quadrature):
    ...
    def __init__(self, p0, p1, n, scheme_fcn=qd.c1.gauss_legendre):
        ...
        super().__init__(scheme=scheme_fcn(n), trans=lambda t: 0.5 * (self.
            p0 + self.p1) + 0.5 * (self.p1 - self.p0) * np.reshape(t, (-1, 
            1)), jac=np.linalg.norm(self.p1 - self.p0) / 2)
...
class Quad_Circle(Quadrature):
    ...
    def __init__(self, r, c, n, scheme_fcn=qd.u2.get_good_scheme):
        ...
        def my_trans(x):
            ...
            return rr + self.c
        super().__init__(scheme=scheme_fcn(n), trans=my_trans, jac=2 * np.
            pi * self.r)
...
class Quad_Tri(Quadrature):
    ...
    def __init__(self, v, n, scheme_fcn=qd.t2.get_good_scheme):
        ...
        assert self.v.shape == (3, 2
            ), 'Vertices must be a 3 by 2 list or numpy array!'
        self.vol = get_vol(self.v)
        super().__init__(scheme=scheme_fcn(n), trans=lambda x: x.T @ self.v,
            jac=self.vol)
...
class Quad_Disk(Quadrature):
    ...
    def __init__(self, r, c, n, scheme_fcn=qd.s2.get_good_scheme):
        ...
        def my_trans(x):
            ...
            rr = np.swapaxes(rr, 0, -2)
            return rr + self.c
        super().__init__(scheme=scheme_fcn(n), trans=my_trans, jac=np.pi * 
            self.r ** 2)
...
class Quad_Rect(Quadrature):
    ...
    def __init__(self, v, n, region_type=True, scheme_fcn=qd.c2.get_good_scheme
        ...
        if region_type:
            ...
            self.v = v
        super().__init__(scheme=scheme_fcn(n), trans=lambda x: transform(x,
            self.v), jac=lambda x: np.abs(get_detJ(x, self.v)) * 2 ** np.
            prod(len(self.v.shape) - 1))

    def make_numpy(self):
        self.points_numpy = self.trans(self.points_ref)
        self.weights_numpy = self.weights_ref * self.jac(self.points_ref)
...
class Quad_Sphere(Quadrature):
    ...
    def __init__(self, r, c, n, scheme_fcn=qd.u3.get_good_scheme):
        self.r = np.array(r)
        self.c = np.array(c)
        super().__init__(scheme=scheme_fcn(n), trans=lambda x: x.T * self.r +
            self.c, jac=4 * np.pi * self.r ** 2)


class Quad_Ball(Quadrature):

    def __init__(self, r, c, n, scheme_fcn=qd.s3.get_good_scheme):
        assert n <= 7, 'The degree of the cubature is not more than 7. Otherwise use nD ball scheme!'
        self.r = np.array(r)
        self.c = np.array(c)
        ...
        def my_trans(x):
            ...
            rr = np.swapaxes(rr, 0, -2)
            return rr + self.c
        super().__init__(scheme=scheme_fcn(n), trans=my_trans, jac=4 / 3 *
            np.pi * self.r ** 3)
...
class Quad_Tet(Quadrature):
    ...
    def __init__(self, v, n, scheme_fcn=qd.t3.get_good_scheme):
        assert n <= 14, 'The degree of the cubature is not more than 14. Otherwise use nD simplex scheme!'
        self.v = np.array(v)
        if self.v.shape != (4, 3):
            self.v = self.v.T
        assert self.v.shape == (4, 3
            ), 'Vertices must be a 4 by 3 list or numpy array!'
        from quadpy.tn._helpers import transform, get_vol
        self.vol = get_vol(self.v)
        super().__init__(scheme=scheme_fcn(n), trans=lambda x: transform(x,
            self.v.T).T, jac=self.vol)
...
class Quad_Cube(Quadrature):
    ...
    def __init__(self, v, n, region_type=True, scheme_fcn=qd.c3.get_good_scheme
        ...
        from quadpy.cn._helpers import transform, get_detJ
        assert n <= 11, 'The degree of the cubature is not more than 11. Otherwise use nD cube scheme!'
        if region_type:
...
class Quad_Pyramid(Quadrature):
    ...
    def __init__(self, v, scheme_fcn=qd.p3.felippa_5):
        ...
        self.v = v
        super().__init__(scheme=scheme_fcn(), trans=lambda x: _transform(x.
            T, self.v).T, jac=lambda x: np.abs(_get_det_J(self.v, x.T)))
...
class Quad_Wedge(Quadrature):
    ...
    def __init__(self, v, scheme_fcn=qd.w3.felippa_6):
        ...
        self.v = np.array(v)
        super().__init__(scheme=scheme_fcn(), trans=lambda x: _transform(x.
            T, self.v).T, jac=lambda x: np.abs(_get_detJ(x.T, self.v)))
...
class Quad_nD_Sphere(Quadrature):
    ...
    def __init__(self, r, c, dim, scheme_fcn=qd.un.dobrodeev_1978):
        ...
        super().__init__(scheme=scheme_fcn(self.dim), trans=my_trans, jac=
            self.vol)
...
class Quad_nD_Simplex(Quadrature):
    ...
    def __init__(self, v, dim, n, scheme_fcn=qd.tn.grundmann_moeller):
        ...
        self.dim = dim
        self.vol = get_vol(self.v)
        super().__init__(scheme=scheme_fcn(self.dim, n), trans=lambda x:
            transform(x, self.v.T).T, jac=self.vol)
...
class Quad_nD_Cube(Quadrature):
    ...
    def __init__(self, v, dim, region_type=True, scheme_fcn=qd.cn.stroud_cn_3_3
        ...
        if region_type:
            ...
            self.v = v
        super().__init__(scheme=scheme_fcn(self.dim), trans=lambda x:
            transform(x, self.v), jac=lambda x: 2 ** np.prod(len(self.v.
            shape) - 1) * np.abs(get_detJ(x, self.v)))
