        ...
        if isinstance(f_i, types.FunctionType):
            ...
            if (f_i, tuple(no_tuple_r)) in NP_LAMBDA_STORE.keys():
                ...
                try:
                    ...
                    pass
                if isinstance(f_i, (float, int)):

                    def loop_lambda(constant):
                        return lambda **x: np.zeros_like(next(iter(x.items(
                            )))[1]) + constant
                    lambdify_f_i = loop_lambda(f_i)
                elif type(f_i) in [type((se.Symbol('x') > 0).subs(se.Symbol
                    ('x'), 1)), type((se.Symbol('x') > 0).subs(se.Symbol(
                    'x'), -1)), bool]:

                    def loop_lambda(constant):
                        if constant:
                            return lambda **x: np.ones_like(next(iter(x.
                                items()))[1], dtype=bool)
                        else:
                            return lambda **x: np.zeros_like(next(iter(x.
                                items()))[1], dtype=bool)
                    lambdify_f_i = loop_lambda(f_i)
                else:
                    try:
                        ...
                        for k in no_tuple_r:
                            if isinstance(k, str):
                                ...
                                kk.append(k)
                        kk = [se.Symbol(name) for name in sorted([x.name for
                            x in kk])]
                        ...
                        def lambdify_f_i(**x):
                            if len(x) == 1:
                                ...
                                v = np.stack([v for v in dict(sorted(x.
                                    items())).values()], axis=-1)
                        ...
                        sp_lambdify_f_i = sp.lambdify([k for k in
                            no_tuple_r], f_i, [NP_SYMPY_PRINTER, 'numpy'])
...
NP_SYMPY_PRINTER = {'amin': _min_np, 'amax': _max_np, 'Heaviside':
    _heaviside_np, 'equal': _equal_np, 'Xor': _xor_np}
