...
class StopCriterion:
    ...
    def __init__(self, metric: str, min_delta: float=0.0, patience: float=0,
        mode: str='min', freq: int=1000, strict: bool=True, monitor_freq:
        Any=None, validation_freq: Any=None):
        self.metric = metric
    ...
    def _check_frequencies(self, metric_dict):
        ...
        if self.target_key == 'monitor' and (self.freq % self.monitor_freq !=
            0 or self.freq == 0):
            raise RuntimeError(
                'Stop criterion frequency should be a multiple of the monitor frequency'
                )
        elif self.target_key == 'validation' and (self.freq % self.
            validation_freq != 0 or self.freq == 0):
            raise RuntimeError(
